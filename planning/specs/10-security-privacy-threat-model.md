...
| A-08 | **Execution journal / undo before-images** | v0: in-memory ring only (`session.dart` L146–152, `_journalMax = 25`); spec'd: device-local, encrypted, never synced (Spec 02 §5.2) | High — before-images hold full prior record content | v0: RAM only (acceptable); persisted form MUST be encrypted device-local per Spec 02 §5.2 **once Spec 01 §8.7 ships** — until then device-local plaintext is accepted under the same D-1 honesty conditions (suite-sync CS-17; Spec 04 §3.1 posture note) |
...
1. **Fix the turn log now (R-02) — location half DONE.** ~~Move `turnlog.jsonl` out of the synced `dataDir` to device-local storage~~ **landed (commit `d956390`):** the turnlog now lives in the app-injected device-local `deviceDir` (`~/.plenara`), off the synced folder, exactly the one-file change predicted here. Remaining: cap or rotate it, and split it into the two research-§14 channels — a content-bearing local debug log (dogfood only, plainly labeled) and a shape-only telemetry form (`string[12]`-style redaction) that is the *only* thing Spec 11 may ever export.
...
| **D-5** | **Prompt assembly from metadata-never-record-content on routing/authoring paths; grounded generative kinds carry record content deliberately, scoped per kind by deterministic code, with journal text additionally behind non-disablable per-session consent.** Registry ownership (suite-sync CS-12): **a new cloud-touching feature must add its row to Spec 08 §5.5 before it ships**; this spec's §5 is the threat-annotated view of that registry and reviews each new row for threat posture. | Spec 02 §7.3; Spec 03 §3.5/§5.6; Spec 05 D3; `G-26`; Spec 08 §5.5 |
