...
| **R-02** | Internal dogfood turn/App logs intentionally contain raw utterances, values, responses, and exception details. This is necessary for current stabilization and is governed by Spec 11's build-channel boundary: device-local; explicit warned export; no automatic upload; external raw capture/export disabled; raw audio/interims forbidden. The turnlog location was fixed by `d956390`; AppLog rotation is now 30 days/100 MB. | Medium (accepted internal exposure) | Spec 11; revisit before any external beta |
