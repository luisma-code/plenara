...
**Status:** Active v0.2 — audited 2026-09-07. Grounded in current secure credential storage, build-channel diagnostics, six-kind egress registry, durable plaintext device-local journals, selective one-time Contacts import, and the external artifact gate. Record/journal encryption, the sensitive-skill marker, and independent Layer-3 authoring review remain explicit release gaps.
...
| A-07 | **Turn log** (`turnlog.jsonl`) | Device-local `deviceDir` (`~/.plenara`, injected by the app — commit `d956390`); **formerly the synced `[dataDir]`**, relocated per R-02 | **High — raw utterances, extracted slots, response text, error stacks** | Plaintext and device-local in internal/development builds, retained under the current bounded diagnostic policy; external builds capture none and purge inherited traces |
| A-08 | **Execution journal / undo before-images** | Device-local `execution-journal.json`, never synced; `Session` also maintains a bounded in-memory projection | High — before-images hold full prior record content | Persisted, atomically replaced, capped at 25 terminal executions, corruption surfaced. Plaintext; no record/journal `CryptoBox` runtime exists. Encryption remains deferred with Spec 01 §8.7 |
...
| A-10 | **Content-search embedding index** (`G-34`) | In-memory only; rebuilt from the complete live record store and never persisted | Medium — embeddings of sensitive content invert to meaning | Deterministic feature-hash vectors live only for the process lifetime; deleted ids are evicted on full rebuild. Any future persisted representation must be device-local and encrypted |
...
The research doc (§8.7) argues both sides honestly: encrypting content at rest protects a stray sync copy or a lost device and may itself *enable* the candor the product needs (A-13); leaving files plaintext keeps them portable and readable without Plenara. The deferred design is **scoped encryption** — content-bearing sensitive records encrypted, structural files plaintext forever — with mappings for `journal_entry`, `contact`, and `interaction`, all through one `CryptoBox` (AES-256-GCM) whose key lives in the platform secure store and travels between devices via the OS keychain's own end-to-end-encrypted sync. This paragraph describes a future boundary, not current protection.
