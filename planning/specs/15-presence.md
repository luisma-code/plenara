...
> **⚠ AMENDED 2026-07-28 (`G-52`).** The Y0 capture gesture is now tap-to-start / tap-to-**send**
> (it previously auto-sent on the engine's final, with a second tap aborting). Abort moved to a ✕
> that exists only while listening. The status line carries a decaying "tap anywhere when you're
> done" affordance for the first few sessions — the presence already says THAT it is listening;
> only "how do I finish?" needs words. D14 amended accordingly.
...
the entity. Y0/Y1 remain the active conversational presence; Settings and Data now carry a Y2 ember,
and record detail carries an inline ember. Listening energy follows normalized system-recognizer mic
level where the platform exposes it; the local Whisper path retains a calm self-shimmer. Clarification
and failure have geometry/luminance expressions in addition to color. The independent persisted
**Still presence** control and OS Reduce Motion both select fixed per-state forms with opacity-only
transitions. Presence semantics name input mode, muted state, and expression. The active production
register is 12 figures, gated by a persisted ≥90-second interval and 3/day budget; ordinary writes use
a sub-300 ms whole-body acknowledgement. The full 52-figure corpus remains an internal sketchbook,
...
- **Frequency cap (normative):** at most **one glyph per turn**; **≥ 90 s** between glyphs; a soft daily budget of **3**, with occasion-priority when the budget contends (occasions of the heart — birthdays, closeness, remembrance — outrank affirmations). The gate persists its last-fire time and per-local-day count atomically, so relaunch cannot reset scarcity. Forced fires exist only for internal preview channels; production turns always enter through the gate.
...
*(v0.4 — shipped coverage.)* All 52 authored definitions remain data in `app/lib/glyphs.dart`, but
only `bell`, `candle`, `check`, `double-check`, `gift`, `heart`, `quill`, `reach`, `settle`, `star`,
`target`, and `undo-loop` are admitted by the production gate. The complete vocabulary and forced
preview are available only when the active build channel permits internal tools. `meaningKey` as a
schema field is still owed (§5A.3); the accompanying reply carries the meaning meanwhile.
...
- **Y2 as shipped:** Settings and Data carry a static edge ember, and record detail carries an inline
  ...
  navigation shell rather than creating a second presence instance.
...
Application surfaces use the matching shared tokens `quick` 180 ms, `acknowledge` 260 ms,
`continuity` 320 ms, and `settle` 520 ms. Durable planner collections animate identity-preserving
insertions/removals through one continuity path; captions and proposal replacement use opacity-only
switches. An 11-frame permanent harness is the release evidence for these transitions.
