...
Spec 07 built the design language for Plenara's *views* — archetypes, turn cards, typography — and gave the voice loop a single piece of persistent chrome: the listening orb (§8.4), "an organic, softly irregular form" with four states. That was the seed. This spec grows the seed into the product owner's actual vision: **the interface is not an app that contains an assistant indicator — it is an entity you are talking with.** The screen at rest *is* the presence: an ethereal, quietly alive field that fluctuates with its speech, shifts hue with effort, has a disembodied visual personality, and — rarely, at moments that earn it — traces a symbolic figure (§5A). Text is not the medium; text *materializes* when needed — read-back, lists, narration aids, captions — and dissolves back into the field. When muted, the presence keeps speaking visually while captions carry the words.
...
**Flutter feasibility.** Positions and velocities live in pooled `Float32List` buffers (no per-mote objects, allocation-free in steady state); the flow field is sampled from a small precomputed curl-noise tile; the draw is **one `drawVertices` (or instanced-quad) call** with additive blending, plus at most one cheap low-resolution **aura underlay** pass — a soft ambient hue wash behind the motes, which is also a candidate mechanism for the §4.3 additive-legibility constraint. Cost is linear in count, hence the fixed budget; and the body itself has no fragment-shader dependency (the aura is a trivial gradient), which *shrinks* the platform risk Q1 carried for the veil.
...
The renderer's job is fixed and glyph-agnostic: sample targets along each stroke at mote spacing, assign a subset of live motes, animate them to targets per the timing fields, hold, release. **No glyph ever requires new render code.** Claude may *select* a glyph for an occasion, or — rarely — *compose* one within this schema for a bespoke moment, subject to every rule in §5A.1 and every fence in §5A.7; it cannot invent rendering, blending, or motion outside the schema. The shipped vocabulary (§5A.8) is curated data, versioned with the app.

### 5A.4 Formation, timing, release

- **Assignment.** The director marks a glyph active in the frame stream (a glyph id + progress scalar ride alongside `PresenceFrame` as an optional annotation — the vector itself is unchanged, §2.4; tiers that don't render glyphs ignore it). The renderer assigns N motes to path-sampled targets — enough for the figure's arc length at roughly one mote per 2–3 px, capped at **15 % of the live mote budget**. Non-assigned motes calm (energy damped ~30 %) and never vanish: the body remains the body while part of it writes.
- **Draw.** Targets activate per-stroke and per-target via `delayMs` staggering, so figures *draw* in stroke order — the eye-dots land, then the smile sweeps left to right — each mote easing in with `p-state` character. Total draw 600–900 ms for core glyphs.
- **Hold.** The completed figure holds `holdMs` (typically 400–700 ms), its motes shimmering at breath amplitude in place.
- **Release.** Always softer than the entrance (Spec 07 §8.2 r3): assigned motes let go over ≥ 1.2 × the draw duration at `m-drift` character, rejoining the flow field mid-current. The figure doesn't vanish; it *dissolves back into the weather*.
- **Timing token.** The whole sequence is the `p-glyph` register (§8.1) — one-shot, never looping, one at a time; it occupies the `p-cue` exclusivity slot (§8.2 r2). A glyph requested while one is active is dropped, not queued — nothing is lost, because no glyph carries meaning alone (§5A.7 fence 1).
- **Ground behind the figure.** While a glyph draws and holds, the field behind it calms and dims one step — the calm-band mechanic (§10.1) at lower strength — so the figure reads as *line against ground*. This is the glyph-side face of the §4.3 additive-legibility constraint: a figure must never compete with the fireball it is made from.
...
- **Frequency cap (normative):** at most **one glyph per turn**; **≥ 90 s** between glyphs; a soft daily budget of **~8**, with occasion-priority when the budget contends (occasions of the heart — birthdays, closeness, remembrance — outrank affirmations; affirmations outrank ornaments, which don't exist anyway).
...
5. **Tier degradation.** T2 renders glyphs fully. T1 renders core glyphs at its reduced count; figures marked † in §5A.8 do not read below ~400 motes and follow their `degrade` policy — usually simplification to a plainer sibling (cake → candle) or a skip. T0 and reduced motion behave per fence 2 — and since T0 is the v1 ship vehicle, **glyphs ship with the v3 rung**, not before (§9.3).
...
| `p-glyph` | draw 600–900 ms (per-stroke staggered ease-in) · hold 400–700 ms · release ≥ 1.2 × draw at `m-drift` character | glyph formation (§5A.4) — one-shot, never looping, occupies the `p-cue` slot |
...
| Tier | Renderer | Where |
|---|---|---|
| **T2 — the murmuration** | the full GPU swarm: 2,000–6,000 motes, one instanced/`drawVertices` pass, flow-field advection in pooled buffers, the aura underlay, glyphs (§2.1, §5A) | v3 organic rung; capable GPUs; the design target |
...
| **T0 — the still presence** | static per-state figures — a fixed constellation per state, cross-fade only (identical to §8.3); no glyphs | reduced motion; power-saver; GPU-distressed; and the v1 ship vehicle |
...
- **Q3 — Vital-ramp anchors, aura design, and additive legibility** await the same visual-design pass as Spec 07 Q2 (real mockups, both themes, contrast-checked against the calm band); mechanisms here are decided regardless. *(Extended v0.2:)* the pass must **select the mechanism(s)** by which the ramp survives additive blending — aura underlay, constrained buildup / density cap, separated anchors with saturation floors (§4.3) — and the full-load ramp-distinguishability golden frame gates the v3 rung (§10 item 7). The mockup's white-out fireball is the failure this Q exists to keep from recurring.
...
- **Q5 — Glyph repertoire calibration** *(repurposed v0.2 — the original keep-or-cut-motes question is moot: the body is motes)*: whether §5A.5's caps feel right in the dogfood (too chatty vs. never seen), whether apt-or-absent is holding in practice (no glyph should ever draw a "why did that appear?" — §5A.1's test, checked against real transcripts), which †-marked figures actually read at T1 counts, and which extended glyphs earn promotion to core — or removal — after the v3 pass.
...
*End of Spec 15 — The Living Presence v0.2*
