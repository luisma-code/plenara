...
**Status:** Active v0.3 — audited 2026-09-07. Test totals are generated by the runner and are not frozen in prose. Per-tier coverage, unclassified import failure, generated 05a conformance counts, secret isolation, calibrated visual-verifier checks, detached-operation/relaunch tests, durable planner-artifact tests, and simulator-only phone verification are live in `tool/precheck.sh` and the focused integration harnesses.
...
**P9.2 — Seam with a fake.** Every OS- or network-facing capability ships behind a thin adapter
interface with a deterministic double, so product decisions are CI-tested and only the razor-thin
platform shim needs a simulator/host smoke. Current examples are `StorageRepository`,
`CloudClient`, `NotificationScheduler`, `SpeechRecognizer`/`SpeechOutput`, and
`PhoneContactsSource`/`RelationshipLauncher`. Every future OS surface follows the same discipline.
...
| **3a. Integration — schema/migration/execution** | Invalid definitions remain inert; migrated records either validate completely or retain exact old bytes; execution intent, crash recovery, conflict-safe targeted undo, and truthful storage failures survive process restart. | Real temporary filesystem repositories and device-local journals; deliberately broken codec/migrator/recovery/conflict detectors must make their test red before the passing result counts. | `schema_registry_test.dart`, `schema_hydration_session_test.dart`, `value_codec_test.dart`, `migration_test.dart`, `execution_coordinator_test.dart`, `durable_session_execution_test.dart` |
...
The pyramid remains core-heavy and the rendered app surface remains the designated growth area. Exact totals come from each precheck run rather than this document.
...
| Speech | `SpeechRecognizer` / `SpeechOutput` | deterministic Flutter doubles + Noop implementations | platform STT/TTS | yes — thin shim only, on the local simulator/host; never Luis's physical iPhone |
| Contacts / communication launch | `PhoneContactsSource` / `RelationshipLauncher` | injected fakes | iOS multi-contact picker / URL launcher | yes — thin shim only, on a local simulator; never Luis's physical iPhone |
...
- **Every seed skill across many inputs.** The full Spec 02 §9 seed set is enumerated from
  `v0/data/skills/` by the test rather than duplicated as a prose count, then runs through resolve
  + execute against varied slots — the "recorded request → expected artifact" idea of research §5
  applied to the deterministic side.
...
The Flutter suite is distributed by surface rather than concentrated in `widget_test.dart`:
Relationships, Todos, Habits, Plan, Library/data, routines, settings, onboarding,
attention/conflict recovery, reply/search cards,
...
2. **The tally is generated, never hand-synced.** `bin/conformance_count.dart` consumes the test runner's JSON protocol, requires exactly 60 examples, prints each tier, and ratchets against `test/conformance-baseline.txt`. Current result: **24 pass / 36 skip** — F 14/20, P 0/20, DF 3/10, DP 7/10.
...
**Status (updated 2026-08-17): instrumentation and per-tier enforcement are live.** `coverage_check.dart` fails deterministic core below 90%, product logic below 80%, transport below 60%, global below 80%, and any new unclassified production file. Current measured tiers are printed on every precheck. The in-process retrieval backend is now classified as product logic and counted. `fixture_inputs.dart` and mixed product/operator `replay_cloud.dart` remain explicit temporary exclusions until replay recording is split; this is a named boundary, not silent denominator loss. Import lint also fails every unclassified production file. The hosted CI runner remains absent; the local precheck is authoritative meanwhile.
...
|---|---|---|
| **A — deterministic core** | `interpreter.dart`, `store.dart`, `reminders.dart`, `dates.dart` | **≥ 90% aggregate** |
| **B — product logic & seams** | `session.dart`, `router.dart`, `planner.dart`, `plan_proposal.dart`, `weekly_review.dart`, `planning_artifact.dart`, `operation_center.dart`, `capability_draft_store.dart`, `embed.dart`, `people.dart`, `generative.dart`, `storage_repository.dart`, `execution_coordinator.dart`, `schema_registry.dart`, `value_codec.dart`, `conversation_ledger.dart`, `turnlog.dart`, `config.dart`, `routines.dart`, `automations.dart`, `reference.dart`, `content_search.dart`, `migration.dart`, `cron.dart` | **≥ 80% aggregate** |
| **C — provider transport** | `claude.dart` | **≥ 60% aggregate** |
...
One pipeline, failing on any step: (1) `dart analyze` (both packages, zero issues — already the local norm); (2) `dart test` in `v0/`; (3) `flutter test` in `app/`; (4) coverage thresholds per §8.3; (5) **import-lint layering gate** — the research §9 dependency rule (UI → BL → Storage → Intelligence, no upward or skipping imports) enforced mechanically; (6) a debug build for the host OS; (7) the 05a conformance count regenerated and compared — a *decrease* in N/60 fails the build. Solo-project note: until a hosted runner exists, the same steps run as a local pre-push script; the gate's authority comes from its content, not its host. **BUILT:** `tool/precheck.sh` runs v0/app analysis, exhaustive import classification, engine tests with tiered coverage, Flutter tests, the declared-external reachability test, host build, real-engine integration suite, tracked-file secret scan, and the conformance ratchet against `v0/test/conformance-baseline.txt` = 24. It clears credential environment variables for hermetic fixture execution. Only the hosted runner remains.**
...
- **D5 — Spec completeness is the generated 05a conformance count N/60**, exact utterances and explicit skips, derived from JSON runner events by `bin/conformance_count.dart`; currently **24/60**. Release milestones cite this number and nothing else.
