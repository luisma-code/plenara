...
- **Every seed skill across many inputs.** All 35 seed skills (Spec 02 §9) run through resolve + execute against varied slots — the "recorded request → expected artifact" idea of research §5 applied to the deterministic side.
...
**Honest status: none of this exists.** There is no CI at all (no `.github/`, no pipeline — verification today is a human running `dart test` / `flutter test` / `dart analyze` per HANDOFF), no coverage run, no lcov artifact, no thresholds, and the import-lint dependency-rule gate the research doc and CLAUDE.md both promise ("dependency rule enforced by import-lint CI gate") is likewise unbuilt. The 1363-green suite is strong, but *which production lines it exercises is currently unmeasured* — exactly the "assumed, not measured" state §9.3 forbids. This section specifies the closure.
...
Dogfooding exists to measure the things CI *cannot*: the make-or-break unknowns that depend on a real human's phrasing distribution against real data (Spec 03 §7.3.4; 05d §6). The instrument is already built and tested: every turn appends `{at, utterance, source, skill, cloud?}` to `<dataDir>/turnlog.jsonl` (`source ∈ corpus | cloud | undo | correction | authored | help | clarify | error`; `cloud` records health per Spec 04 §3.5 kinds), and `bin/turnlog_report.dart` aggregates it (unit-tested in `turnlog_test.dart` — the instrument itself obeys P9.1). The tracked metrics, in priority order:
