...
**Status (updated 2026-08-17): instrumentation and per-tier enforcement are live.** `coverage_check.dart` fails deterministic core below 90%, product logic below 80%, transport below 60%, global below 80%, and any new unclassified production file. Current measured tiers are printed on every precheck. `fixture_inputs.dart`, the localhost-only `embed.dart`, and mixed product/operator `replay_cloud.dart` are explicitly excluded until retrieval is in-process and replay recording is split; this is a named boundary, not silent denominator loss. Import lint now also fails every unclassified production file. The hosted CI runner remains absent; the local precheck is authoritative meanwhile.
...
| **B — product logic & seams** | `session.dart`, `router.dart`, `people.dart`, `generative.dart`, `storage_repository.dart`, `turnlog.dart`, `config.dart`, `routines.dart`, `automations.dart`, `reference.dart`, `content_search.dart`, `migration.dart`, `cron.dart` | **≥ 80% aggregate** |
