...
```json
{
  ...
  "params": { "contact": "Sarah", "budget": 50 },
  "confidence": 0.88,
  "routingSource": "cloud_model"
}
```
...
**Tier, cloud, and why *generation* has no fast path — but *recognition* does (v0.7).** Every generative kind requires Claude, so `generative_request` is BYOK-gated exactly as the capability-definition meta-intents are (§2.2): on the free tier the router still *produces* the intent and the orchestrator surfaces the paid-upgrade prompt (Spec 05 §3.6) rather than spending a shared key. Two costs must be split, because v0.5 conflated them:

- **Generation is never fast-pathed.** It is always a multi-second **detached** cloud call (Spec 04 §3.10, §4.7); a learned entry could never skip it, so the corpus does not cache generations and a generative turn writes no records.
- **Recognition *is* learnable, and must be.** Recognizing that "suggest a gift for Elena" is a `gift_ideas` request is a routing decision — the same classification the corpus was built to memoize. A learned **recognition template** (surface shape → `generativeKind`, with `{e:contact}` placeholders — *shapes not values*, §5.4) earns its keep: it (a) skips the residual Haiku classification round-trip on the user's own habitual phrasings, (b) survives the §3.5 residual rate cap, (c) converts the *offline* failure of an unrecognized request from a false "I didn't catch that" into GenerativeService's honest "I can't reach the cloud for that" (P2.8 — recognition is what buys the truthful degrade), and (d) is the **free tier's only keyless path** to the §2.2a upgrade prompt, since the BYOK-gated residual (§3.5) never runs without a key, so the offline recognizers — the frozen regexes (`G-44`) and learned templates — are the sole producers of a `generative_request` there.

So the write paths (§2.6) now serve generative turns in a **recognition-only** mode: the learned-template *target* widens from `skillId` to `skillId | generativeKind` (§5.2), but generation itself still never touches the corpus. This is the literal shape of "Claude evolves the local handling to meet the user": the cloud residual recognizes a novel phrasing once, and the DSL absorbs it — no per-miss regex edit.
...
### 2.3a Reference-by-Number (numbered readbacks + corrections) — `G-49`

A misheard item ("Zpack my clothes") is nearly impossible to re-target by text, because the user cannot force STT to reproduce the same mis-hearing. So **every list Plena reads back is numbered** ("1. …, 2. …"), and the user references an item **by the number spoken** — resolving to the exact record, never a fuzzy text match. This is the same registry-code pre-filter class as §2.3 (deterministic, offline, zero model calls); it is gated on a live enumeration context and otherwise falls through to normal routing.

- **The enumeration channel (Spec 02 §3).** The DSL ops `enumerate` (flat lists) and `ref_mark` (one item from inside a `foreach`, for rich/conditional/joined readbacks) render the numbered lines **and**, from the same pass, export an ordered `{id, typeId, labelField, spokenLabel}` list — so numbering and reference resolution can never drift, and a **single readback may mix record types** (facts + relationships) with each item carrying its own type/field.
- **The conversational context (`_enumCtx`).** Populated after a readback dispatches; **survives intervening unrelated turns** (the marquee flow is "list … *(aside)* … delete 2"); **replaced** by the next readback; **cleared** by an empty readback (never leave a stale referent); **never persisted** (in-memory, dropped on app close). References resolve by `recordId`; labels are re-derived live at reference time (so a corrected/undone item self-heals).
- **The three actions**, recognized by ordered regexes (number | ordinal | "last") only when the context is live: **delete N**, **complete N** (gated to types with a `completed`/`done` boolean — otherwise an actionable refusal), and **correct N** — a two-turn re-speak (`"correct 1"` → *"what should it say?"* → the next utterance replaces the label field, dictation taken verbatim so a command-shaped replacement isn't misrouted; `"never mind"` cancels) plus a one-turn form (`"change 2 to X"`). All three journal a before-image, so the `undo` system command (§2.3) reverses them like any spoken write. Out-of-range, vanished-record, already-done, and no-active-list all end at a visible surface (P2.8).
- **Why session code, not a DSL skill or the corpus.** The enumeration context is *conversational state* a pure `(skill, slots, store)` skill cannot see, and a type-generic complete/correct is inexpressible as one static-`typeId` skill; the recognition must stay free, offline, and immune to corpus-learning noise. This is the registry-code carve-out (§2.2a's `G-44` posture), not a routing decision left to a model.
...
> **v0.7 contract delta (`G-46`).** The write paths and `ClarificationNeeded.partialIntent` above are typed to `SkillInvocation`; v0.7 widens them to a named union **`RoutedRecognition = SkillInvocation | GenerativeRequest`**, so a residual-recognized `generative_request` reaches `recordConfirmation`/`recordCorrection` (recognition-only, §2.7) and a missing generative param can be a pending `partialIntent` (§6.3). Nothing else in the signatures changes; a generative `RoutedRecognition` carries a `generativeKind` where a `SkillInvocation` carries a `skillId`, never both. A v0 generative confirmation is always `ConfirmationKind.implicit` — no clarification surface offers generative candidates until retrieval ships (§7.3.2).
...
**Turn lifecycle for a `skill_invocation`** (other categories resolve at the seam: `system_command` → app shell; `clarification_needed` → prompt then `resolveFollowUp`; `define_*` → authoring subsystem, tier-gated; `generative_request` → `GenerativeService` as a detached, read-only, tier-gated operation, Spec 04 §3.6/§3.10 — no resolve/execute pass, and a **recognition-only** corpus write-back (v0.7): generation writes no records, but the *recognition* is learnable (§2.2a) — a **delivered**-and-uncorrected generative reply is an implicit `recordConfirmation` against its recognition template (target `generativeKind`), and a next-turn `"correct"` is the §5.2 negative half (forget). Since a generative turn has no execute/`done`, **delivered** is defined as *GenerativeService returned success and the synthesis was rendered to the user*. A turn that **errored/degraded** (cloud failure, rate-limit, invalid key, barge-in cancel) or that only surfaced the **free-tier upgrade prompt** shows the user nothing to judge the recognition against — so it takes the write-back rules' existing **"Neither"** branch: no confirm, no learn (an uncorrectable turn is not evidence the recognition was right). A content-quality complaint voiced as `"correct"` still forgets the template — the same accepted asymmetry the skill path carries):
...
**Compound utterances (§10 MD8) — amended to the shipped v0 behavior.** The earlier sketch here (dispatch the primary, *queue* the residual, defer simultaneous multi-intent) is superseded: v0 ships **two** compound mechanisms, and simultaneous multi-intent is no longer deferred — it executes as a single batched turn.

- **Coordinator-seam split (offline, corpus-only).** After the whole utterance fails to route as one command, the orchestrator scans for the first `" and "` / `", and "` seam where **both** sides independently corpus-route into **fully-slotted** skills (a half missing a required slot disqualifies the seam — a mid-compound follow-up can't be composed into one confirmation). Both halves dispatch in the same turn and the confirmations compose. The conservative both-halves test is what keeps "remind me to buy milk and eggs" — one command containing "and" — unsplit, and detection is pure (no dispatch, no writes), so declining to split costs nothing.
- **Multi-sentence decomposition (cloud).** A **compound** utterance — two or more *substantial* sentences, each with ≥2 real words — is detected *before* corpus matching and handed straight to the cloud router. The detector splits on terminal punctuation followed by whitespace, with an abbreviation/initial guard ("St. Jude", "5 p.m.", "J. R.", "e.g." — their periods are stripped before splitting) so decimals, titles, and initials never false-split; a single trailing mark is not a compound. Bypassing the corpus here is load-bearing: a greedy corpus `{…:text}` slot would otherwise swallow the whole blob into ONE wrong skill ("I just had dinner with X. I learned Rina is going to UW" matched `i just had {food:text}` and logged two sentences as a meal). The cloud router **decomposes** the utterance into a multi-action batch (e.g. an interaction + a relationship + a fact), each action a fully-slotted `skill_invocation`.

**Batch execution rules (multi-action turn).** Per action: slots are sanitized (§6.5) and typed-normalized against the skill's declared inputs (§6.5); an action naming an unknown skill or missing a required slot is **skipped and counted** — the composed reply admits "I couldn't record N parts of that", never a silent drop (P2.8, and no per-action slot follow-up inside a batch); two actions that normalize to the **same (skillId, resolved slots)** de-duplicate to one record (a cloud split can emit the same record twice); one action's failure is contained and counted, never aborting the batch after earlier actions persisted. The per-action journal entries **collapse into one undo unit**, so `undo` / "no, I meant" reverses the whole turn, not just the last record. **No template is learned from a compound turn** — a compound is not one phrasing. `route` stays single-intent *per action* (§2.5); the batch is the orchestrator-level composition of several.
...
- A persisted admission controller inside the single `ClaudeClient` HTTP door allows at most **200 calls per local calendar day** and **30 calls per rolling 10 minutes**. It reserves before HTTP, so routing, generation, authoring, figure generation, connection tests, retries, and relaunches cannot bypass the same guard. Above either limit it returns typed `CloudError.rateLimited`; Settings shows both counters. These are deliberately generous internal-dogfood defaults, replacing the stale 20/hour onboarding-hostile proposal. Recalibrate from beta cost/latency evidence before an external build.
...
> — mechanically, or via a cloud-**suggested** surface abstraction validated by round-trip
> (§5.2, v0 mechanics) — and **inserted at full trust after one clean (uncorrected) use** —
> it fast-paths every similar phrasing thereafter; a `"correct"` on the very next turn
> **forgets** (removes) the learned template that routed it, including one learned *that same
> turn* from a cloud misroute. Seed (shipped) templates take precedence over learned ones
...
> **v0 shipped learning mechanics (the binary ratchet's write path, `G-45`).** A cloud-routed single-action turn that dispatches cleanly learns a template *that same turn*, by one of two routes, tried in order:
>
> 1. **Cloud-suggested template (`learnSuggested`).** The routing call may return a **surface-abstracted suggested template** alongside the routed action — this is what lets date/time phrasings be learned at all (the *resolved* value, e.g. `2026-07-14`, is not in the surface text, so mechanical value-substitution can't abstract it). The router **trusts the abstraction but validates it by round-trip** before adopting: the template must (a) contain ≥1 placeholder (a zero-slot template is just a memorized utterance) and **≤4** placeholders (many free-text groups invite catastrophic regex backtracking — a ReDoS cap); (b) use only the **closed slot-type vocabulary** (§5.4) — any unknown type is rejected, never compiled; (c) name every placeholder — an ignore-named `{_:…}` slot is rejected (it produces no slot, so the round-trip can't validate it); (d) keep **≥1 strong literal word** — a word of ≥2 letters that is not a stop-word (`i`, `my`, `the`, `is`, `to`, …) — so a catch-all (`{x:text}`) or near-catch-all (`i'm {mood:text}`) can never be learned, persisted, and hijack routing; (e) not already be in the corpus; and (f) **compile, match the original utterance, and re-extract — via the same extraction path the router uses at dispatch (§6.4) — to exactly the slots the turn dispatched** (per-key string comparison over the union of keys). A malformed pattern is rejected, never thrown.
> 2. **Mechanical fallback (`learn`).** When no suggestion was offered or it failed a guard: each non-null slot *value* is found verbatim in the surface and replaced with a typed placeholder. **Every** non-null slot must abstract — a lossy template would both drop a slot and persist a private slot *value* verbatim into the synced corpus (violating "store slot shapes, not values"). A value that is a known contact abstracts to `:contact`, never `:text` (preserving the contact guard across learns, §5.4); the same strong-literal and dedupe guards apply.
>
> A learned template is appended to `corpus-learned.json`, marked learned, and matched only in `route`'s **second pass** — seeds always win (§4.2), so a learned entry can never shadow a built-in. The **negative half**: if the very next turn is a correction ("no, I meant …"), the learned template that routed the previous turn — *including one freshly learned that same turn from a cloud misroute* — is forgotten (removed from the in-memory corpus **and** from `corpus-learned.json`), so a bad pattern can't re-route future utterances. Only learned templates are forgettable; a seed is never removed this way. A multi-action compound turn never learns (§2.7).
>
> **Generative recognition targets (`G-46`, v0.7).** The learned-template *target* is `skillId | generativeKind` — a residual-recognized `generative_request` (§2.2a) learns its surface shape → kind exactly as a skill route does, through both routes above (a cloud-suggested abstraction preferred; the mechanical `{e:contact}` fallback otherwise), under all the same guards (≥1 strong literal, closed slot-type vocabulary, round-trip validation, seeds-win, forget-on-`correct`, no-learn-on-compound). The only difference is the confirmation signal: a generative turn has no execute/`done`, so **delivered-and-uncorrected** stands in for `recordConfirmation` (§2.7). Generation is still never cached — only the recognition is.

### 5.3 Entry lifecycle & the shared signature

A Lane-1 entry is `{ templateSig, skillId, slotRecipes, confidence, originSource, lastUsedAt, useCount }` — with, in v0.7 (`G-46`), the **target field mutually-exclusive**: an entry carries **either** `skillId` **or** `generativeKind` (never both; the on-disk v0 shape is `{ "skillId", "template", "slots"? } | { "generativeKind", "template" }`, never `"skillId": "gift_ideas"` — the namespaces stay distinct, §2.2a). A `generativeKind` entry has no `slotRecipes` beyond the `{e:contact}`-style param placeholders and never routes to the interpreter. Its `templateSig` — `(normalized-intent, typeIds, slot-shape)` — is the **signature shared with Lane 2** (§5.1): one key schema across the two homes, not one byte stream. Lifecycle: **created** (at `initConf*`, §4.2) → **boosted** on uncorrected reuse → **decayed** on disuse (§4.2) → **invalidated** (§5.5). Routing consults Lane 1 first (the §4.3 cascade, `Axis 3` corpus trust): a template match at trust ≥ `θ_corpus_act` dispatches and describes with no model call; in the moderate band it dispatches with the transparent-routing caveat; below `θ_corpus_drop` it is not a hit and falls through to retrieval (§7.3).
...
A generative-recognition template (`G-46`, §5.2/§5.3) uses this **same machinery**: its one `{contact:entity}` param placeholder is a normal `entity`-typed slot from the closed vocabulary, abstracted and re-extracted exactly like a skill's `entityRef` slot — the only difference is the entry's *target* (`generativeKind`, not `skillId`).
...
> **v0 shipped template DSL (`corpus.json` / `corpus-learned.json`).** The concrete form of the sketch above. A corpus entry is `{ "skillId", "template", "slots"? }`. The template is the literal carrier phrase with `{name:type}` placeholders (`{_:type}` is an *ignore* slot — matched but never extracted; rejected in learned templates, §5.2). It compiles to a **case-insensitive anchored regex** with a named capture group per slot — literal runs join on `\s+`, and one trailing `.`/`?`/`!` is tolerated so terminal punctuation never leaks into a slot — so v0 matching is anchored-regex, not the literal hash of the v1 sketch. The **slot-type vocabulary is CLOSED** — currently thirteen types; a template naming any other type is rejected at learn time and a new type is a code change (`_compile` + this table), not a data drop:
>
> | type | capture pattern | resolution |
> |---|---|---|
> | `text` | free text (`.+?`) | surface string, verbatim |
> | `entity` | `.+?`, but the **first token must not be** an article/pronoun/determiner (`the`, `a`, `my`, `i`, `that`, …) — a name slot can't swallow "remember **the alamo**" (G-12) | surface string; entity resolution happens in the skill via `read_one` |
> | `contact` | `.+?`, valid **only if** the capture is a known contact name/alias (lowercased set passed per turn) | surface string — the guard is what lets a broad "what is {who:contact} {q:text}" seed coexist with world-knowledge OOD ("what is the capital…" — "the" is not a contact) |
> | `quantity` | free text | first numeric token → `num` |
> | `date` | free text | §6.2 resolver, **literal year kept** — a birthday anchor stays as spoken and rolls forward at *read* time via `next_annual` |
> | `futuredate` | free text | §6.2 resolver, **forward-intent** (`preferFuture`): a bare month-day already past this year rolls to **next** year (used for `create-task.dueDate`, reschedule — no due dates in the past) |
> | `datetime` | free text | §6.2 datetime resolver; a time-of-day is **required** — no clock time → null (the task-vs-reminder discriminator, §6.4) |
> | `dayword` | **bounded word class**: `today`, `tomorrow`, `tonight`, `this morning/afternoon/evening`, `[next] <weekday>` | §6.2 resolver, forward (bare weekday → the *next* occurrence) |
> | `pastday` | bounded: `yesterday`, `today`, `[last] <weekday>` | **backward-looking**: a bare weekday → the **previous** occurrence, strictly before today (used for `log-interaction.at` — a logged past event never gets a future date) |
> | `posword` | bounded: ordinal words, `last`, plain/ordinal digits | surface (a list position — keeps "the **first** task" positional while "the **milk** task" falls through to by-description) |
> | `moodword` | closed feeling-adjective list | surface (so "i'm {mood:moodword}" can't swallow "i'm going to…" or "i'm 180 lbs") |
> | `mealword` | closed meal-name list (`breakfast`, `lunch`, `dinner`, `snack`, …) | surface |
> | `predword` | closed relational-predicate list (`is allergic to`, `works at`, `lives in`, …), longest-first | surface (paired with a `:contact` subject so it only annotates known people) |
>
> The bounded word-class types exist because the matcher validates **one** regex match and never backtracks across alternative splits: a greedy free-text day sitting between two other slots would grab the wrong span and fail; a bounded pattern makes the only viable split the correct one.
>
> **Fixed-slot constants.** An entry may carry a constant `"slots"` object — e.g. `"had dinner with {personName:entity}"` carries `{ "kind": "dinner" }` — merged into the extracted slots **at match time** with default semantics: a regex-captured slot always wins; the constant only fills a slot the surface didn't supply (§6.4). This is how an **offline** corpus template records a categorical value ("had dinner/lunch/coffee with X" → `kind=dinner/lunch/coffee`, "texted X" → `kind=text`) without a free-text slot to extract it from — the slot is asserted by the phrasing itself. Empty for the vast majority of entries.
...
- **Directional resolution (v0, shipped)** — the resolver runs in three modes, keyed by the *slot type* (§5.4), because a bare day expression is ambiguous without the intent's direction:
  - `date` — the **literal** resolution: a month-day stays in the current year even if past, so a birthday anchor is stored as spoken and rolls forward at *read* time via `next_annual` (the `G-15` ownership split below).
  - `futuredate` — **forward-intent** (`preferFuture`): a bare month-day that already fell earlier this year rolls to **next** year — a due date in the past is never what "add a todo for March 3" means. Used for `create-task.dueDate` and reschedules.
  - `pastday` — **backward-looking**: a bare weekday ("Tuesday") resolves to the **previous** occurrence, strictly before today ("yesterday"/"today"/"last friday" already resolve correctly and pass through). Used for logging past events (`log-interaction.at`), so a logged interaction never gets stamped with a future date.
  The same three entry points serve the cloud path (`resolveDate` / `resolveFutureDate` / `resolvePastday`) so cloud-routed raw strings get identical date math (§6.5) — the model is never trusted to compute the date itself.
...
A **generative_request**'s `params` take the same loop (`G-46`, v0.7): a kind whose required param is absent — "suggest a gift" with no contact — halts before the (paid) generation call and asks *"Gift ideas for whom?"*, the pending record carrying `generativeKind` in place of `skillId`. Contact resolution reuses the one path skills use: the residual's bounded, quoted `knownContacts` clause (so Haiku returns the exact contact name) plus the `GenerativeService`'s store-side resolver (which owns the unknown-contact surface — "I don't have Elena as a contact yet"). Never silently guess a param (P2.8).

### 6.4 Slot recovery from a corpus template *(the `span`/`fixed` recipes, as shipped in v0)*
A corpus hit re-extracts its slots **deterministically from the compiled template — no inference** (this is what the `span`/`fixed` recipe references in §4.1/§5.3/§7.3.3 resolve to in v0). Each named capture group resolves per its slot type (the §5.4 table: date types through the §6.2 resolver, `quantity` to a `num`, `text`/`entity`/word-class types as surface strings). Two rules make the match itself a *validator*, not just a matcher:

- **A typed slot that fails to resolve means the template does not apply.** An unparseable `date`/`futuredate`/`dayword`/`pastday`/`datetime` capture, a `:contact` capture that is not a known contact/alias, or an `:entity` capture whose first token is a stop token (§5.4) rejects the entry, and matching **falls through to the next template** rather than dispatching with a null. For `datetime` this is the **task-vs-reminder discriminator**: "call mom on friday" carries no clock time → the `datetime` reminder template resolves null and is skipped → the date-only `create-task` template wins.
- **Fixed slots merge last, as defaults.** After extraction, the entry's constant `"slots"` object (§5.4) folds in — a regex-captured slot always wins; the constant only fills a slot the surface didn't supply.

The exact same extraction path validates a cloud-suggested template's round-trip before it is learned (§5.2), so a learned entry is admitted by precisely the code that will route it at dispatch time.

### 6.5 The slot sanitizer & cloud typed-slot normalization *(v0, shipped)*
Two normalization passes sit between **any** router's raw slot output and resolution/persistence:

- **Sanitizer (all sources — corpus, cloud, and every action of a multi-action batch, §2.7).** A blank/whitespace-only string, or the sentinels `"none"` / `"null"` (case-insensitive), normalizes to a **real `null`** before it reaches a resolver or is persisted — the cloud sometimes fills an absent *optional* slot with a sentinel, and without this a whitespace-only "note"/"kind" would be stored as a present value and the skill's own `isNull` branches would never fire. Non-string values pass through untouched.
- **Typed normalization (cloud path only).** The corpus types its slots via the template, but the cloud returns raw strings; for every input the routed skill declares as `date` / `futuredate` / `datetime` / `pastday`, the raw string is resolved through the §6.2 deterministic resolver with the matching direction — the model is never trusted with the date math itself. An unresolvable **required** datetime becomes null and drops into the skill's own "when?" clarify branch rather than persisting a wrong or unparseable time (a cloud `"2026-07-08"` for a `datetime` slot → null — no accidental midnight reminders; a raw `"tomorrow at 3pm"` → a real ISO datetime — no silently-dropped reminders).
...
> **Current bounded act lane (Increment 3).** The 40–47% generic top-1 result still forbids naked retrieval dispatch. A retrieval winner may act only when it clears the calibrated similarity+margin gate **and** its candidate has a deterministic, candidate-specific slot extractor that fully recovers required user data. The first shipped lane is held-out task capture; every candidate without such an extractor falls through to the full-inventory Haiku residual or visible clarify. Thus retrieval contributes before cloud without reinstating the rejected “top-1 alone is truth” design.
...
> **Kind-tagged residual inventory (`G-46`, v0.7).** In-process retrieval is enabled by default and its bounded deterministic lanes front-run `routeResidual`; the residual remains Haiku-as-router over the full inventory for every unresolved novel phrase, per §7.3.4. That inventory is now **kind-tagged**: skills *and* the fixed generative kinds (§2.2a). The response contract accordingly admits a **`{"generativeKind","params"}`** shape as a top-level single route beside `{"skillId","slots"}` / `{"actions":[…]}`; a `generativeKind` outside the closed set is treated as abstain (parallel to the invented-`skillId` rule — enforced by the same `none`/abstain filter this section defines). `routingSource: cloud_model` is a legal source for a `generative_request` (§2.5). A generative kind may **not** appear as an action *inside* a multi-action `{"actions":[…]}` batch in v0: §2.7's batch execution is skill-shaped (journal entries, one undo unit, de-dup on `(skillId, slots)`), so a mixed utterance's generative half is **skipped-and-counted** ("I couldn't do N parts of that", P2.8), and no template is learned from a compound turn anyway (§2.7).
...
**✅ EVALUATED (findings §13; design slate in `05d-routing-design.md`).** A candidate redesign was measured on a held-out set. The adopted router: **multi-vector retrieval** (one vector per example phrase, not one concatenated vector) + **Claude-authored synthetic anchor phrases** — held-out **80% top-1 / 96% recall@5** (vs the 47% single-vector floor), which restores the online candidate-set path. For **online** cold-start, **Haiku over the full skill inventory** (no top-K truncation) measured **94%** at ~$0.0004/turn — so the top-8 truncation above is dropped for online users. A deterministic **act-type gate was tested and rejected — overfit** (it dropped held-out accuracy 15 pts and gated the correct skill out of 28% of cases). The **corpus-learning ratchet** was simulated: friction falls from ~20% (cold) to **~3%** as habitual phrasings are learned — **conditional on real per-user phrasing reuse**, which is the make-or-break unknown for the offline/free tier and must be measured in a beta. The residual hard case is the task/reminder/recurring **twins** (a slot-signal tiebreak is the untested lever).

**Increment-3 implementation delta.** The production backend is now an in-process deterministic feature-hashing representation, not the earlier localhost experiment and not yet the proposed packaged bge checkpoint. It builds the multi-vector skill index on startup with no service dependency. Its accepted-dispatch surface is intentionally narrower than the findings-13 architecture: only a high-margin winner with a fully deterministic candidate-specific extractor may act. The held-out task-capture lane is calibrated in the repository; broader lanes and replacement by a packaged sentence transformer require their own held-out accuracy/margin gate before activation.
