...
> **Kind-tagged residual inventory (`G-46`, v0.7).** In the v0 implementation retrieval is off (§3.3 `retrieval: false` on-device), so the residual (`routeResidual`) is Haiku-as-router over the full inventory, per §7.3.4. That inventory is now **kind-tagged**: skills *and* the fixed generative kinds (§2.2a). The response contract accordingly admits a **`{"generativeKind","params"}`** shape as a top-level single route beside `{"skillId","slots"}` / `{"actions":[…]}`; a `generativeKind` outside the closed set is treated as abstain (parallel to the invented-`skillId` rule — enforced by the same `none`/abstain filter this section defines). `routingSource: cloud_model` is a legal source for a `generative_request` (§2.5). A generative kind may **not** appear as an action *inside* a multi-action `{"actions":[…]}` batch in v0: §2.7's batch execution is skill-shaped (journal entries, one undo unit, de-dup on `(skillId, slots)`), so a mixed utterance's generative half is **skipped-and-counted** ("I couldn't do N parts of that", P2.8), and no template is learned from a compound turn anyway (§2.7). When retrieval ships it front-runs the residual unchanged — the kind-tagged candidates simply arrive from the index instead of the prompt.
