...
**Folder authority as shipped.** Settings labels the default mobile root **Device-local only**. Choosing a location uses the platform folder picker. On iOS the picker starts a provisional security-scoped grant and returns its path without replacing the active bookmark. Plenara then copies through a sibling staging directory, validates the complete target, and proves it writable. Only after those steps succeed does one commit door promote the native bookmark and Dart configuration; finalization releases the prior grant. Any validation, copy, probe, native-commit, configuration, or finalization failure rolls the bookmark and configuration back to the prior authority. The bridge also handles a first-ever switch, where “prior bookmark” is deliberately absent. A non-empty existing Plenara root is adopted rather than overwritten; an incomplete or unrelated root is rejected. Desktop uses the system folder chooser. The app never guesses that its sandbox Documents directory is synced.
...
v0's `ensureSeeded` (`config.dart`) records the decision: on first run against an empty folder, the shipped seed **types, skills, templates, and seed corpus are copied into the synced folder**, and everything accretes there thereafter. On later launches it adds missing built-ins and promotes only explicitly newer shipped type schemas through the backed-up path below. Runtime hydration still treats an installed seed type identically to an authored one, reading both from `types/`; “built-in” is installation provenance, not a privileged runtime class.

Two consequences this spec makes explicit:

- **Built-in installation is idempotent and upgrade-aware.** `ensureSeeded` runs on every launch from the definitions bundled in that binary. Missing built-ins are copied into an already-populated folder, so an older install cannot strand a newly referenced type or skill. Existing authored files and same-version user edits remain the synced truth. Legacy definitions without an explicit version are backed up and normalized as v1 without replacing their content. A shipped type advances only when its bundled `schemaVersion` is newer: the exact installed definition is first retained under `.seed-backups/types/`, the newer definition is written atomically, and records then move through the normal contiguous migration path (Spec 01 §5.2 step 3). Corpus and same-version definitions are never recopied over user state.
- **Templates are shipped content that becomes folder content.** Spec 01 §12.4 (`G-22`) calls templates "binary-shipped `(type + bundled skills)` pairs"; v0 materializes them into `templates/` at seeding so `instantiate-template` (Spec 02 §9) reads them uniformly through `loadDefs`. Reconciliation: the *authoritative source* of a template remains the app binary (a new install always has the current set); the folder copy is a materialization that may lag after an app update. **The read path, precisely (suite-sync CS-22):** `loadDefs` serves templates from the folder materialization (uniformity — Spec 02 §9 reads templates like any definition); at *instantiation* the registry compares the template's version against the binary's and **re-materializes from the binary first if the binary is newer**, then instantiates from the folder — so uniformity and binary authority compose rather than conflict. Template files are never user-edited; a template conflict is resolved by re-materializing from the binary (no §6.3 escalation needed).
...
| `_meta.fieldTombstones` | **FIELD-level tombstones (`G-50`)** — `field → HLC stamp of its removal`, present only when a field has been cleared. A cleared field previously vanished from both `fields` and `stamps`, leaving the merge nothing to compare, so a peer's older value for that field would **resurrect** — the clear silently un-did itself. Stamped **once**, at the moment of removal, and carried forward unchanged (re-stamping on every write would reintroduce the same bug in mirror image); the key is dropped as soon as the field is written again. Reachable in v0 today: clearing an optional value in the Library removes the key. |

**Per-field change detection is structural, not identity (`G-50`).** A field is re-stamped only when its value actually differs from the value on disk, compared **deeply**. Dart's `==` on `List`/`Map` is identity, so `tag`/`list`/`json` fields never compared equal to their own reloaded selves and were re-stamped on *every* write — collapsing precisely the fields per-field stamps exist to protect back into whole-record LWW, and letting a device that touched an unrelated field win the list field spuriously.
