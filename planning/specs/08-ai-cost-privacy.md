...
- **Implemented app posture (2026-08-17):** one `CredentialStore` interface backed by iOS Keychain, the native macOS login Keychain (through `/usr/bin/security`, so unsigned dogfood builds do not require a provisioning profile), Android Keystore-backed secure storage, and the platform Windows secure implementation. The key is device-local, never synced, and rejected at the logging boundary in every diagnostic channel (Spec 11). A legacy plaintext config key migrates only after a secure write is read back and verified; then the plaintext field is atomically cleared. Failure preserves the source value. The macOS helper's noninteractive write necessarily carries the value in that helper's short-lived process arguments; Plenara never logs the command or arguments, and same-user process inspection remains inside R-03's accepted unlocked-device boundary (Spec 10).
- **Development compatibility:** `ANTHROPIC_API_KEY` remains a process-lifetime override only in the `development` build channel and is never copied into secure storage. Internal/TestFlight and external builds ignore it. Pure-Dart v0 operator tools retain their explicit local config/environment seam; the distributed Flutter app does not.
