...
| Functional-gap feedback / diagnostics | — (no model; channel, trigger, payload, and consent all per **Spec 11 §§4–5**, the owning spec — this row exists so the registry stays exhaustive, suite-sync CS-25) | per Spec 11 §§4–5 | per Spec 11 §§4–5 (structurally content-free by construction) | per Spec 11 §§4–5 |
