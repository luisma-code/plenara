...
- **Typed outcomes.** Every call returns `CloudOk<T>` or `CloudError(kind, detail)` with `kind ∈ {noKey, offline, timeout, badKey, rateLimited, serverError, malformed}`. `Ok(null)` from the router is a *real answer* (the model abstained — "not one of my capabilities"), distinct from any failure. The v0 client maps HTTP 401/403 → `badKey`, 429 → `rateLimited`, socket failures → `offline`, and bounds the whole exchange with a 30 s timeout — it **never throws** (`claude.dart`).
...
| STT / TTS (Spec 06) | — (platform on-device engines) | **Never** (Plenara-side; platform-native engines are configured on-device per Spec 06) | Nothing via Plenara | – |
...
| `briefing` (Spec 05 §15) | Haiku | On ask, or scheduled fire-time (never batch, §4.4) | Date; open task descriptions; active reminder texts; upcoming-birthday nudge lines | b (invocation) / b-standing for the scheduled automation, granted when the user enables the briefing automation |
...
- **D6 — Batch API only for deadline-free work** (the weekly consolidation pass); the briefing generates at fire-time. *(Reaffirms research §7.2 amendment / Fable F-11; §4.4.)*
...
- **Q7 — Generative-kind registry drift.** v0 ships kind-specific system prompts for only three kinds (`gift_ideas`, `briefing`, `reconnect`), with the rest falling to a generic grounded prompt, and adds `draft_message`, which is absent from Spec 03 §2.2a's fixed set. Reconcile the fixed set across Spec 03 §2.2a / Spec 04 §3.10 / this spec's §3.3 table and ship a reviewed prompt per kind — the closed-set discipline only means something if the set is actually closed.
