...
**Status:** v0.4 — amended 2026-08-17. This is the sole authority for diagnostic collection/export. Development and single-user internal builds deliberately retain content-bearing logs—including speech-recognizer hypotheses needed to reconstruct a failed capture—and permit explicit raw export; external builds capture no raw content and expose no raw export. Secrets and raw audio are forbidden in every channel. The compiled-artifact gate and exact internal export preview are implemented. Decisions recorded in §10.
...
The current single-user dogfood build exports the retained `.log` files **and the device-local `turnlog.jsonl`** without removing user content. The turnlog is included because it is the turn-outcome instrument the `.log` files cannot reconstruct, and because on iOS it lives in a dot-directory the Files app hides — leaving it out made it unretrievable from the phone at all. Both carry the same Class C content under the same warning. The Settings action is named **Share raw diagnostics** and immediately states that the export contains conversation text, record values, and exception details. Before the system share sheet opens, a second explicit preview names every included file, the exact payload size, revision, content classes, and maximum retained log text; cancel sends nothing. The manifest records revision, channel, platform, app/schema versions, and every included filename. Export is manual; no uploader exists.

Retention is 30 days or 100 MB, whichever threshold is crossed first, oldest-first. Internal raw traces may include interim recognizer hypotheses under P11.2; the export preview's conversation-content warning covers them. Raw audio is never written. Class S filtering occurs before serialization, not while composing the export.
...
- **AppLog:** one file per run; internal/development prune older than 30 days and oldest-first above 100 MB, and the size pass never deletes the most recent previous run's log — that is the one an engineer booted to read. iOS uses its app Documents diagnostic folder for manual retrieval; desktop uses its device-local diagnostic directory. External creates no raw file and purges inherited internal `.log` files **and any inherited `turnlog.jsonl`** on startup.
- **turnlog:** written only when the build channel captures content — the repository takes an explicit `enableTurnlog` flag and external builds construct it `false`, so the content-bearing turn record does not exist in an external build rather than merely being hidden from its UI. Every line crosses the Class S rejection boundary before serialization in **every** channel (registered secrets, `sk-ant-*`, bearer/`x-api-key` headers, private-key blocks), so an exception message that interpolates a credential cannot reach the file. Unbounded append today; already device-local (D9 landed, commit `d956390`); v1 rolls it into monthly segments with a total cap. It is the input to the make-or-break metrics (§8), so it is kept generously — but locally.
