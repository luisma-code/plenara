...
- The Relationships workspace opens on a bounded Focus list of genuinely due people rather than every saved name. Urgency chooses which eight people enter the bounded Focus set; every displayed people list—Focus, expanded Focus, search, circle/proximity filters, and All—is alphabetized by name. Each person row carries an icon-and-text health badge plus a matching avatar ring: sage `Healthy` with days until the next goal, amber `Due today` or `Due in Nd`, coral `Nd overdue` or no-history copy, and neutral `Not tracked`. Color is redundant rather than the only signal, and overdue age is measured against that person's chosen contact rhythm so different circles remain comparable. Global search reaches everyone; faceted circle and proximity filters show membership and due counts and combine as `Close + Local` or `Close + Remote`. Every person row and multi-select Organize flow can change either axis through one durable, undoable execution. Circle changes apply inherited goals; proximity changes never alter contact frequency. Local/Household guidance leads to planning time together, while Remote guidance leads to a call or FaceTime. A relationship signal on Todos opens the named person's actionable detail workspace directly; overload and stale-queue signals open Plan with the represented tasks selected so schedule/defer actions are immediately available. Deterministic signals run before AI and are capped at two on Todos. Engagement counts explicit suggestion decisions and relationship follow-through, never impressions, opens, animation, or screen time.
...
Adding a follow-up emits an ordinary contact-linked task into Todos and uses proximity to make the action concrete: make plans for Local/Household people, plan a call or FaceTime for Remote people, and use a neutral reach-out for unknown proximity. Opening a system communication app does not fabricate an interaction; the user logs the completed connection explicitly.
