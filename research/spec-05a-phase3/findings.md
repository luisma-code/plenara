...
## 13. Routing redesign, evaluated (05d candidates E1–E5) — measured verdicts

Fable's 05d design pass proposed a candidate slate; this is the evaluation (harness: `probe_05d.py`, `heldout_eval.py`, `eval_haiku_fullinv.py`, `learning_curve.py`). Run independently of the design (by Opus, not Fable); **held-out utterances generated by Sonnet** — deliberately *not* the Haiku model that wrote the synthetic anchors — to avoid same-model style leakage.

### E1–E2 · Multi-vector + anchors holds on held-out; the act-type gate is overfit
The 05d probe's headline (80% top-1 with a deterministic act-type **gate**) reproduced on the *training* set — but that gate's regex was written while looking at the data. On a **held-out** set (59 fresh Sonnet utterances, frozen router):

| config | training top-1 | **held-out top-1** | held-out recall@5 |
|---|---|---|---|
| multi-vector max-sim | 51% | 74.5% | 92% |
| **multi + synthetic anchors (no gate)** | 69% | **80.4%** | **96%** |
| multi + anchors **+ act-type gate** | **80%** | **64.7%** ↓ | 72.5% ↓ |

- **ADOPT `R1`+`R2`** (multi-vector index + Claude-authored anchor phrases): robust at **80% top-1 / 96% recall@5** on unseen data. The single→multi-vector index change alone lifts recall@5 from 80% to ~96%, restoring the online Haiku path from the §12 ceiling (~69%) to ~recall@5 × 86% ≈ **82%**.
- **REJECT `R3` as written** (deterministic act-type gate): overfit — it *drops* held-out accuracy 15 pts and wrongly gates the correct skill out of **28% (14/51)** of cases (recall collapses to 72%). The *concept* may survive with rules that generalize, but the current regex must not ship, and any gate needs its own held-out validation. Fable's own caveat was exactly right.

### E4 · Haiku full-inventory solves online cold-start (`R8`)
Hand Haiku the full 17-skill inventory + the utterance (no retrieval truncation): **94.1% routing on held-out, 100% OOD-abstention, ~$0.0004/turn.** The only misses are the genuinely-ambiguous task/reminder/recurring twins. For **online** users, cold-start is essentially solved — no recall ceiling, cheap, self-extinguishing as the corpus learns.

### E5 · The corpus-learning curve — the make-or-break, holds *conditionally*
Offline/free users get on-device retrieval only (80%), so ~20% of novel phrasings misroute until the corpus learns. Simulated one persona's usage (300 turns, Zipfian phrasing reuse, base = the E2 winner, learn-on-use):

| corpus mode | corrections/300 | early-third | late-third | corpus-hit | errors |
|---|---|---|---|---|---|
| none (retrieval only) | 82 | 32% | 25% | 0% | 0 |
| **exact/near-exact (θ=0.995)** | **27** | 17% | **3%** | 71% | 0 |
| soft (θ=0.86, `R9b`) | 27 | 17% | 3% | 71% | 0 |
| soft (θ=0.82, `R9b`) | 28 | 16% | 3% | 75% | **3** |

- **The corpus-learning mechanism delivers the vision** — friction falls from ~17–28% (cold) to **~3%** (learned) — **conditional** on users reusing habitual phrasings. With *maximum* phrasing variety (no reuse, an earlier run), the corpus barely fires and friction stays ~20–28%. **The real per-user phrasing-reuse distribution is the single most important unmeasured variable** and needs real usage data (a beta) to settle.
- **Exact/near-exact corpus already suffices** (71% hits, 3% late friction, 0 errors); **soft generalization (`R9b`) is a marginal, risky refinement** (75% hits but 3 false-positive routes at θ=0.82). Ship exact/near-exact first; gate soft on evidence.

### Verdict
- **Adopt:** `R1` (multi-vector), `R2` (synthetic anchors + linter), **bge-small** embedder, `R8` (Haiku full-inventory online cold-start), corpus learning (exact/near-exact).
- **Reject / redesign:** `R3` (act-type gate — overfit).
- **Defer, evidence-gated:** `R9b` (soft corpus), `R5`–`R7` (learned ranker, cross-encoder, fine-tuned embedder).
- **Residual hard problem:** the task/reminder/recurring **twins** — wrong in *both* retrieval and Haiku; a deterministic slot-signal tiebreak (recurrence shape / explicit time — `R4`) is the untested lever.
- **Make-or-break unknown:** real per-user phrasing-reuse — measure in a beta; it decides whether offline/free reaches "rarely asks."
- **Caveats:** 17-skill floor; model-generated utterances (not real speech); the learning sim is a model with stated assumptions; small n. All numbers directional, not production SLAs.
