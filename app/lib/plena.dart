...
class _GlyphRun {
  ...
  late final List<int> travellers; // shuffled traveller indices, consumed as deposits
...
class _PresenceViewState extends State<PresenceView> with SingleTickerProviderStateMixin {
  ...
  double _coreX = 0, _coreY = 0; // Plena's centre of mass (flies the path during a glyph)
  _GlyphRun? _run;
  final _f = _Frame();
  ...
  void didUpdateWidget(covariant PresenceView old) {
    ...
    _sync();
    if (_animating && widget.glyph != null && widget.glyphNonce != old.glyphNonce) {
  ...
  Offset _along(List<Offset> pts, List<double> cum, double tot, double u) {
    ...
    return Offset(pts[i - 1].dx + (pts[i].dx - pts[i - 1].dx) * t, pts[i - 1].dy + (pts[i].dy - pts[i - 1].dy) * t);
  }
  ...
  Offset _corePath(_GlyphRun r) {
    ...
    for (var i = 0; i < r.tf.length - 1; i++) {
      if (r.ms < r.tf[i + 1]) {
        ...
        return Offset(r.tx[i] + (r.tx[i + 1] - r.tx[i]) * f, r.ty[i] + (r.ty[i + 1] - r.ty[i]) * f);
      }
    }
  ...
  void _startGlyph(GlyphDef g) {
    ...
    for (final d in g.dots) {
      for (var k = 0; k < 20; k++) {
        final a = _rng.nextDouble() * math.pi * 2, r = math.sqrt(_rng.nextDouble()) * .05;
        pts.add([d.at.dx + math.cos(a) * r, d.at.dy + math.sin(a) * r, d.delayMs + k * 3.0]);
      }
    }
    for (final s in g.strokes) {
      final cum = <double>[0];
      var tot = 0.0;
      for (var i = 1; i < s.pts.length; i++) { tot += (s.pts[i] - s.pts[i - 1]).distance; cum.add(tot); }
    ...
    for (var i = 0; i < cap; i++) {
      ...
      run.tx.add(pts[j][0]); run.ty.add(pts[j][1]); run.tf.add(pts[j][2]); run.filled.add(false);
    }
    final idx = List<int>.generate(_n, (i) => i);
    for (var i = _n - 1; i > 0; i--) { final j = _rng.nextInt(i + 1); final t = idx[i]; idx[i] = idx[j]; idx[j] = t; }
    final nt = (_n * 0.70).floor();
    for (var i = 0; i < _n; i++) { _mode[i] = 0; _trav[i] = 0; }
    for (var i = 0; i < nt; i++) { _trav[idx[i]] = 1; }
  ...
  void _step() {
    ...
    if (run != null) {
      ...
      if (run.ms < run.lastFill) {
        ...
        final w = math.sin((run.ms - run.lastFill) * .03) * .05; // a pleased flitter at the end
      ...
      for (var i = 0; i < run.tf.length; i++) { // deposit: hand a travelling mote to the shape
        if (run.filled[i] || run.ms < run.tf[i]) continue;
        while (run.dc < run.travellers.length) {
          final pi = run.travellers[run.dc++];
          if (_mode[pi] == 0) { _mode[pi] = 1; _tgt[pi * 2] = run.tx[i]; _tgt[pi * 2 + 1] = run.ty[i]; run.filled[i] = true; break; }
        }
      }
      if (!run.flit && run.ms >= run.flourishStart) {
        run.flit = true;
        for (var i = 0; i < _n; i++) {
          if (_trav[i] == 1 && _mode[i] == 0) { _p[i * 4 + 2] += (_rng.nextDouble() - .5) * .055; _p[i * 4 + 3] += (_rng.nextDouble() - .5) * .055; }
        }
      }
      if (!run.released && run.ms >= run.holdEnd) { run.released = true; for (var i = 0; i < _n; i++) { _mode[i] = 0; _trav[i] = 0; } }
      if (run.ms >= run.endAt) { _run = null; for (var i = 0; i < _n; i++) { _mode[i] = 0; _trav[i] = 0; } }
    } else {
      _coreX += (0 - _coreX) * .05;
      _coreY += (0 - _coreY) * .05;
    }
...
class _PlenaPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < _PresenceViewState._n; i++) {
      ...
      double a, sz;
      if (gActive && s._mode[i] == 1) { // deposited: Plena's TAIL — faint, wispy, holds the figure
        a = (.19 * f.luminance + .06).clamp(0.0, .28);
        ...
        final feather = (1 - (math.max(0, dl - rti * 1.1)) / .62).clamp(0.0, 1.0);
        a = ((.05 + .11 * f.energy) * feather).clamp(0.0, .62);
