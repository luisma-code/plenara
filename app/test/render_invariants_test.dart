...
class _Metrics {
  final double
  luminance; // mean luminance over the whole frame (motes are additive over dark)
  final double
  coverage; // fraction of pixels lit clearly above the warm-black ground
...
Future<_Metrics> _renderState(
  WidgetTester tester,
  PresenceState state, {
  PresenceExpression expression = PresenceExpression.neutral,
}) async {
  final key = GlobalKey();
  await tester.pumpWidget(
    Directionality(
      ...
      child: MediaQuery(
        data:
            const MediaQueryData(), // static frame comes from animate:false, not reduced-motion
        child: Center(
...
void main() {
  testWidgets(
    'presence states are visually distinguishable + obey the spec orderings (§15 X7)',
    (tester) async {
      ...
      for (final m in [idle, listening, thinking, speaking]) {
        expect(
          m.coverage,
          greaterThan(0.02),
          reason: 'a state rendered nearly nothing: $m',
        );
      }
      ...
      expect(
        ...
        reason:
            'speaking should read clearly brighter than thinking — $speaking vs $thinking',
      );
      expect(
        speaking.coverage,
        greaterThan(listening.coverage),
        reason:
            'speaking should spread broader than the tighter listening — $speaking vs $listening',
      );
      ...
      double sep(_Metrics a, _Metrics b) =>
          ...
          ((a.coverage - b.coverage).abs());
      expect(
        sep(speaking, thinking),
        greaterThan(0.02),
        reason: 'speaking≈thinking — states collapsed',
      );
      expect(
        sep(speaking, idle),
        greaterThan(0.015),
        reason: 'speaking≈idle — states collapsed',
      );
      expect(
        sep(thinking, listening),
        greaterThan(0.01),
        reason: 'thinking≈listening — states collapsed',
      );
    },
  );

  testWidgets(
    'failure expression changes shape and luminance without hue alone',
    (tester) async {
      final neutral = await _renderState(tester, PresenceState.speaking);
      final failure = await _renderState(
        tester,
        PresenceState.speaking,
        expression: PresenceExpression.failure,
      );
      expect((failure.luminance - neutral.luminance).abs(), greaterThan(.004));
      expect((failure.coverage - neutral.coverage).abs(), greaterThan(.006));
    },
  );
}
