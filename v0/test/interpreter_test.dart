...
(Plan, Map<String, Map<String, dynamic>?>) _run(String skillId,
    Map<String, dynamic> slots, Map<String, Map<String, dynamic>> store) {
  final i = _i();
...
Map<String, dynamic> _workout(
        String id, String activity, dynamic distance, String date) =>
    {
      'id': id,
      'typeId': 'workout',
      'activity': activity,
...
void main() {
  group('schema enforcement at write time (AI authors, code executes)', () {
    test('a numeric-looking decimal slot uses the exact decimal representation',
        () {
      final i = _i();
      final skill = {
        'skillId': 'x',
    ...
    test(
        'a value that cannot be a number is a clean ResolveError, never a junk write',
        () {
      final i = _i();
      final skill = {
        'skillId': 'x',
      ...
      expect(() => i.resolve(skill, {'d': 'quite far'}, _store()),
          ...
          reason: 'surfaced (P2.8), not silently written');
    });
  });

  group('read_related honours orderBy/orderDir/limit', () {
    test(
        'newest-first + limit 1 returns the LATEST related record, not store order',
        () {
      ...
      expect(p.confirmation, '1:2026-07-04',
          reason: 'newest first, then limit 1');
    });
  });

  group('the static gate closes the branch-condition vocabulary', () {
    test(
        "a filter op used as a branch cond ('lte') is rejected at AUTHORING time",
        () {
            ...
            'main': [
              {
                ...
                'var': 'x',
                'value': 1
              }, // the var-closure check is separate and real
              {'op': 'branch', 'cond': c, 'then': [], 'else': []},
              {'op': 'format', 'template': 'ok', 'into': 'confirmationText'},
            ]
          },
        };
        expect(() => _i().validateSkill(skill), returnsNormally,
            reason: '$c must be accepted');
      }
    });
  });

  group('compute — arithmetic accepts canonical numeric values', () {
    test('add parses exact decimal strings rather than concatenating them', () {
      // Exact decimals are stored as strings; arithmetic must still produce a number.
      expect(_i().compute('add', ['5', '1'], {}), 6);
      expect(_i().compute('add', [5, 1], {}), 6);
      expect(_i().compute('add', [null, 1], {}), 1,
          reason: 'null still counts as 0');
  ...
  group('compute (closed fn vocabulary)', () {
    ...
    test('today -> ISO date',
        () => expect(_i().compute('today', [], {}), '2026-07-06'));
    test(
        'format_date EEEE -> weekday',
        () => expect(_i().compute('format_date', ['2026-07-09', 'EEEE'], {}),
            'Thursday'));
    test(
        'format_date other -> date only',
        () => expect(_i().compute('format_date', ['2026-07-09', 'y'], {}),
            '2026-07-09'));
    test('format_date null -> null',
        () => expect(_i().compute('format_date', [null, 'EEEE'], {}), isNull));
    test(
        'format_time afternoon -> 12h clock',
        () => expect(_i().compute('format_time', ['2026-07-09T17:05:00'], {}),
            '5:05 PM'));
    test(
        'format_time noon -> 12 PM',
        () => expect(_i().compute('format_time', ['2026-07-09T12:00:00'], {}),
            '12:00 PM'));
    test(
        'format_time midnight -> 12 AM',
        () => expect(_i().compute('format_time', ['2026-07-09T00:00:00'], {}),
            '12:00 AM'));
    test(
        'format_time morning -> AM',
        () => expect(_i().compute('format_time', ['2026-07-09T09:30:00'], {}),
            '9:30 AM'));
    test('format_time null -> null',
        () => expect(_i().compute('format_time', [null], {}), isNull));
    // clock is Monday 2026-07-06 (see _i())
    test(
        'next_annual later this year',
        () => expect(
            _i().compute('next_annual', ['2020-12-25'], {}), '2026-12-25'));
    test(
        'next_annual already passed -> next year',
        () => expect(
            _i().compute('next_annual', ['1990-03-03'], {}), '2027-03-03'));
    test(
        'next_annual today -> today',
        () => expect(
            _i().compute('next_annual', ['1990-07-06'], {}), '2026-07-06'));
    test(
        'days_until_annual future',
        () =>
            expect(_i().compute('days_until_annual', ['2000-07-16'], {}), 10));
    test('days_until_annual today -> 0',
        () => expect(_i().compute('days_until_annual', ['2000-07-06'], {}), 0));
    test(
        'days_until_annual passed -> rolls to next year',
        () =>
            expect(_i().compute('days_until_annual', ['2000-07-05'], {}), 364));
    test('next_annual null -> null',
        () => expect(_i().compute('next_annual', [null], {}), isNull));
    // streaks over a record list's date field (clock is Monday 2026-07-06)
    test(
        'current_streak today + back',
        () => expect(
            _i().compute('current_streak', [
              [
                {'date': '2026-07-06'},
                {'date': '2026-07-05'},
                {'date': '2026-07-04'}
            ...
            3));
    test(
        'current_streak alive via yesterday',
        () => expect(
            _i().compute('current_streak', [
              [
                {'date': '2026-07-05'},
            ...
            2));
    test(
        'current_streak broken (only older days)',
        () => expect(
            _i().compute('current_streak', [
              [
                {'date': '2026-07-04'}
            ...
            0));
    test(
        'current_streak dedupes same-day',
        () => expect(
            _i().compute('current_streak', [
              [
                {'date': '2026-07-06'},
            ...
            1));
    test(
        'longest_streak finds the max run',
        () => expect(
            _i().compute('longest_streak', [
              [
                {'date': '2026-07-01'},
                {'date': '2026-07-02'},
                {'date': '2026-07-03'},
            ...
            3));
    test('streak over empty list -> 0',
        () => expect(_i().compute('current_streak', [[], 'date'], {}), 0));
    ...
    test(
        ...
        () => expect(
            ...
            3));
    test('avg empty -> null (no data, not a misleading 0)',
        () => expect(_i().compute('avg', [[], 'd'], {}), isNull));
    test('min / max', () {
      expect(
          _i().compute('min', [
            [
    ...
    test(
        'days_between',
        () => expect(
            _i().compute('days_between', ['2026-07-06', '2026-07-10'], {}), 4));
    test(
        'add_days',
        () => expect(
            _i().compute('add_days', ['2026-07-06', 5], {}), '2026-07-11'));
    test('if ternary', () {
      expect(_i().compute('if', [true, 'a', 'b'], {}), 'a');
      expect(_i().compute('if', [false, 'a', 'b'], {}), 'b');
    });
    test(
        'start_of_week(Wed) -> Monday',
        () => expect(
            _i().compute('start_of_week', ['2026-07-08'], {}), '2026-07-06'));
    test(
        'start_of_week(Sun) -> Monday',
        () => expect(
            _i().compute('start_of_week', ['2026-07-12'], {}), '2026-07-06'));
    test(
        'start_of_week(Mon) -> same',
        () => expect(
            _i().compute('start_of_week', ['2026-07-06'], {}), '2026-07-06'));
    for (final c in [
      [3, 4, 7],
      [0, 5, 5],
      [10, 0, 10],
      ...
      test('add ${c[0]}+${c[1]}=${c[2]}',
          () => expect(_i().compute('add', [c[0], c[1]], {}), c[2]));
    }
    test('add with null -> treats as 0',
        () => expect(_i().compute('add', [5, null], {}), 5));
    test('mul', () => expect(_i().compute('mul', [6, 7], {}), 42));
    test('div', () => expect(_i().compute('div', [10, 4], {}), 2.5));
    test('div by zero -> null (guarded, no crash/Infinity)',
        () => expect(_i().compute('div', [5, 0], {}), isNull));
    test('div non-number -> null',
        () => expect(_i().compute('div', ['x', 2], {}), isNull));
    test('round', () => expect(_i().compute('round', [2.6], {}), 3));
    test('percentage idiom: round(100 * part/whole)', () {
      final pct = _i().compute('round', [
        _i().compute('mul', [
          100,
          _i().compute('div', [30, 50], {})
        ], {})
      ], {});
      expect(pct, 60);
    });
    ...
    test(
        'start_of_month',
        () => expect(
            _i().compute('start_of_month', ['2026-07-14'], {}), '2026-07-01'));
    test('read_reference looks up a shipped dataset by key (Spec 13)', () {
      ...
      final i = Interpreter(const {}, DateTime(2026),
          references: {'nutrition': store});
      final skill = {
        'skillId': 'x',
        'inputs': [
          {'name': 'food'}
      ...
      expect(i.resolve(skill, {'food': 'a banana'}, {}).confirmation,
          startsWith('105 cal'));
      // a miss -> ref is null -> the field is empty, no crash
      expect(i.resolve(skill, {'food': 'nothing'}, {}).confirmation,
          startsWith(' cal'));
    });
    test(
        'format renders {name} AND the model-variant {var:name} (authoring robustness)',
        () {
      final skill = {
        'skillId': 'x',
        'inputs': [
        ...
        'steps': {
          'main': [
            {
              'op': 'format',
              'template': 'Logged {count} / {var:count}.',
              'into': 'confirmationText'
            }
          ]
        }
    ...
    test('concat',
        () => expect(_i().compute('concat', ['a', 'b', 'c'], {}), 'abc'));
    test('concat skips null',
        () => expect(_i().compute('concat', ['x', null, 'y'], {}), 'xy'));
    test(
        'unknown fn -> throws',
        () => expect(
            () => _i().compute('bogus', [], {}), throwsA(isA<ResolveError>())));
  });
  ...
  group('val (value resolution)', () {
    ...
    test(
        ...
        () => expect(
            ...
            'c1'));
    test('{ref} to null -> null',
        () => expect(_i().val({'ref': 'p'}, {}), isNull));
    test(
        '{field}',
        () => expect(
            _i().val({
  ...
  group('cond', () {
    ...
    test('isNull false',
        () => expect(_i().cond({'isNull': 'x'}, {'x': 1}), isFalse));
    test('notNull true',
        () => expect(_i().cond({'notNull': 'x'}, {'x': 1}), isTrue));
    test('notNull false',
        () => expect(_i().cond({'notNull': 'x'}, {}), isFalse));
    test(
        'gte dates equal (inclusive)',
        () => expect(
            _i().cond({
              'gte': ['2026-07-06', '2026-07-06']
            }, {}),
            isTrue));
    test(
        'gte dates before',
    ...
    test(
        ...
        () => expect(
            _i().cond({
              'gte': ['2026-07-20', '2026-07-06']
            }, {}),
            isTrue));
    test(
        'gte numeric single digit',
        () => expect(
            _i().cond({
              'gte': [5, 1]
            }, {}),
            isTrue));
    test(
        'gte numeric multi-digit (fixed: 10>=3)',
        () => expect(
            _i().cond({
              'gte': [10, 3]
            }, {}),
    ...
    test(
        ...
        () => expect(
            _i().cond({
              'contains': ['She likes CHESS', 'likes chess']
            }, {}),
            isTrue));
    test(
        'contains miss',
        () => expect(
            _i().cond({
              'contains': ['plays piano', 'chess']
            }, {}),
            isFalse));
    test(
        'contains empty needle never matches',
        () => expect(
            _i().cond({
              'contains': ['anything', '']
            }, {}),
    ...
    test(
        'unknown cond -> throws',
        () => expect(
            () => _i().cond({'bogus': 1}, {}), throwsA(isA<ResolveError>())));
  });
  ...
  group('read_many — ordering, limit, filter operators', () {
    ...
    String? run(List<Map<String, dynamic>> main) => _i().resolve({
          'skillId': 'x',
          'steps': {'main': main}
        }, {}, store()).confirmation;
          ...
          run([
            {
              'op': 'read_many',
              'typeId': 'workout',
              'filter': {'field': 'date', 'op': 'gte', 'value': '2026-07-06'},
              'into': 'recent'
            },
            {
              'op': 'compute',
          ...
          run([
            {
              'op': 'read_many',
              'typeId': 'workout',
              'filter': {'field': 'activity', 'op': 'neq', 'value': 'run'},
              'into': 'nonRuns'
            },
            {
              'op': 'compute',
  ...
  group('read_one — alias tier (G-24)', () {
    ...
    test('resolves a nickname via aliases',
        () => expect(who('the boss'), 'Sarah'));
    test('resolves a comma-listed alias', () => expect(who('Mum'), 'Sarah'));
    test('exact displayName still wins (no alias needed)',
        () => expect(who('Tom'), 'Tom'));
    test('an unknown name matches nothing (no false alias hit)',
        () => expect(who('Nobody'), 'none'));
  });
  ...
  group('create-task — NLU slots (with due date -> weekday label)', () {
    for (final entry in _weekday.entries) {
      test('due ${entry.key} -> "${entry.value}"', () {
        final (p, _) = _run('create-task',
            {'description': 'X', 'dueDate': entry.key}, _store());
  ...
  group('count-runs-this-week — aggregation over many stores (read-only)', () {
    final scenarios = <String, (List<Map<String, dynamic>>, String)>{
      'two runs this week': (
        [
          _workout('a', 'run', 5, '2026-07-06'),
          _workout('b', 'run', 3, '2026-07-07')
        ],
        '8'
      ),
      'run last week excluded': (
        [
          _workout('a', 'run', 5, '2026-07-06'),
          _workout('b', 'run', 10, '2026-06-28')
        ],
        '5'
      ),
      'walk excluded by activity': (
        [
          _workout('a', 'run', 5, '2026-07-06'),
          _workout('b', 'walk', 2, '2026-07-06')
        ],
        '5'
      ),
      'empty store': (<Map<String, dynamic>>[], '0'),
      'boundary: run exactly on weekStart counts': (
        [_workout('a', 'run', 4, '2026-07-06')],
        '4'
      ),
      'day before weekStart excluded': (
        [_workout('a', 'run', 9, '2026-07-05')],
        '0'
      ),
      'decimals sum': (
        [
          _workout('a', 'run', 3.5, '2026-07-06'),
          _workout('b', 'run', 2.5, '2026-07-08')
        ],
        '6.0'
      ),
      'null distance adds nothing': (
        [
          _workout('a', 'run', null, '2026-07-06'),
          _workout('b', 'run', 4, '2026-07-07')
        ],
        '4'
      ),
      'many runs': (
        [
          _workout('a', 'run', 2, '2026-07-06'),
          _workout('b', 'run', 3, '2026-07-07'),
          _workout('c', 'run', 4, '2026-07-08'),
          _workout('d', 'run', 1, '2026-07-09')
        ],
        '10'
      ),
    };
  ...
  group('remember-person-fact — resolve-or-create + entityRefs', () {
    test('new person, no relation -> contact + fact; fact.subject = minted id',
      ...
      final (p, _) = _run('remember-person-fact',
          {'personName': 'Mia', 'fact': 'likes tea'}, store);
      ...
      expect(fact['subject'],
          contact['id']); // entityRef by resolved id, not the name
    ...
    test('existing person -> resolved, no new contact', () {
      ...
      final (p, _) = _run('remember-person-fact',
          {'personName': 'Sarah Mitchell', 'fact': 'runs marathons'}, store);
      expect(
          p.writes.map((w) => w['typeId']), ['contact_fact']); // no new contact
      expect(p.writes[0]['subject'], 'c1');
    });
    test(
        'with relation, new relative -> contact + fact + relative + relationship',
        () {
      final store = _store();
      final (p, _) = _run(
    ...
    test(
        'resolves an existing person case-insensitively (no duplicate contact)',
        () {
      final store = <String, Map<String, dynamic>>{
      ...
      final (p, _) = _run('remember-person-fact',
          {'personName': 'mia', 'fact': 'likes tea'}, store);
      expect(p.writes.map((w) => w['typeId']),
          ['contact_fact']); // NOT [contact, contact_fact]
      expect(p.writes[0]['subject'], 'c1');
    });
    test('with relation, existing relative -> resolved (no 2nd contact write)',
      ...
      expect(p.writes.map((w) => w['typeId']),
          ['contact', 'contact_relationship', 'contact_fact']);
  ...
  group('recall-facts', () {
    ...
    test('known with facts', () {
      ...
      expect(p.confirmation,
          isNot(contains('someone else'))); // filtered by subject
    });
    test('known with no facts', () {
      final store = {
        'c1': {'id': 'c1', 'typeId': 'contact', 'displayName': 'Tom'}
      };
  ...
  group('static validation — the authoring gate', () {
            ...
            'main': [
              {
                'op': 'write_record',
                'typeId': 'contact_fact',
                'fields': {'subject': subjectField, 'fact': 'y'},
                'into': 'f'
              }
            ]
          }
    ...
    test('reject: entity fed by a raw {var}', () {
      expect(() => _i().validateSkill(skillWriting({'var': 'x'})),
          throwsA(isA<ResolveError>()));
    });
    test('reject: entity fed by a literal', () {
      expect(() => _i().validateSkill(skillWriting('some-id')),
          throwsA(isA<ResolveError>()));
    });
    test('reject: entity fed by {ref} to a non-record var', () {
      expect(() => _i().validateSkill(skillWriting({'ref': 'ghost'})),
          throwsA(isA<ResolveError>()));
    });
  ...
  group('update + delete ops (Fable review)', () {
    test('write_record with target updates an existing record (merge, same id)',
      ...
      final (p, before) =
          _run('complete-task', {'description': 'buy milk'}, store);
      ...
      expect(before['task-x'],
          isNotNull); // update -> before is the prior (undo can restore)
    ...
    test('delete_record removes from store and captures the before-image', () {
      ...
      final (p, before) =
          _run('delete-task', {'description': 'buy milk'}, store);
      expect(p.deletes, ['task-x']);
      expect(store.containsKey('task-x'), isFalse);
      expect(
          before['task-x']!['description'], 'buy milk'); // undo can restore it
    });
    test(
        'read_related returns records whose via-attr points at the from record',
        () {
      final store = <String, Map<String, dynamic>>{
        'c1': {'id': 'c1', 'typeId': 'contact', 'displayName': 'Mia'},
        'f1': {
  ...
  group('hardened authoring gate (Fable review)', () {
    Map<String, dynamic> skill(List main) => {
          'skillId': 'x',
          'steps': {'main': main}
        };
    ...
    test(
        'enumerate/ref_mark validation gate (G-49): malformed ops are rejected at authoring',
        () {
      // enumerate needs list + label + into
      expect(
          () => _i().validateSkill(skill([
                {'op': 'enumerate', 'label': 'description', 'into': 'lines'},
                ok
              ])),
          throwsA(isA<ResolveError>()));
      expect(
      ...
      expect(
          () => _i().validateSkill(skill([
                {'op': 'ref_mark', 'typeId': 'nope', 'id': 'a', 'field': 'f'},
                ok
              ])),
          throwsA(isA<ResolveError>()));
      expect(
          () => _i().validateSkill(skill([
                {'op': 'ref_mark', 'typeId': 'task', 'field': 'description'},
                ok
              ])),
          throwsA(isA<ResolveError>()));
    ...
    test('unknown compute fn (e.g. median, unimplemented) rejected', () {
      expect(
          () => _i().validateSkill(skill([
                {'op': 'compute', 'fn': 'median', 'args': [], 'into': 'x'},
                ok
              ])),
          throwsA(isA<ResolveError>()));
    });
          ...
          () => _i().validateSkill(skill([
                {
                  'op': 'read_many',
                  'typeId': 'task',
                  'filter': {'field': 'dueAt', 'op': 'bogus', 'value': 1},
                  'into': 'ts'
                },
                ok
              ])),
    ...
    test('missing steps.main rejected without crashing', () {
      expect(() => _i().validateSkill({'skillId': 'x'}),
          throwsA(isA<ResolveError>()));
      expect(() => _i().validateSkill({'skillId': 'x', 'steps': {}}),
          throwsA(isA<ResolveError>()));
    });
    test(
        'branch-leak: a var resolved only in then, used after the branch, is rejected',
        () {
      final s = skill([
        {
          'op': 'branch',
    ...
    test(
        'validateType rejects unknown valueType + entity without refType; accepts valid',
        () {
      expect(
          () => _i().validateType({
                'typeId': 't',
                'attributes': [
                  {'name': 'x', 'valueType': 'bogus'}
                ]
              }),
          throwsA(isA<ResolveError>()));
      expect(
    ...
    test('all seed skills declare and satisfy their reads/writes', () {
      for (final sk in _skills.values) {
        expect(sk['reads'], isA<List>(),
            reason: '${sk['skillId']} must declare reads');
        expect(sk['writes'], isA<List>(),
            reason: '${sk['skillId']} must declare writes');
        expect(() => _i().validateSkill(sk), returnsNormally,
            reason: sk['skillId'] as String);
      }
    });
    test(
        'mint produces unique ids across 500 fresh interpreters (no cross-session collision)',
        () {
      final ids = <String>{};
      for (var k = 0; k < 500; k++) {
        final p = Interpreter(_types, _now)
            .resolve(_skills['create-task']!, {'description': 'x'}, _store());
        ids.add(p.writes.first['id'] as String);
      }
      expect(ids.length, 500,
          reason: 'every minted id must be unique across sessions');
  ...
  group('error paths (resolve/execute throw ResolveError, not crash)', () {
    ...
    test('read_one ambiguous (G-12)', () {
      ...
      expect(() => _run('recall-facts', {'personName': 'Mia'}, store),
          throwsA(isA<ResolveError>()));
    });
    test('unknown op', () {
      final s = {
    ...
    test(
        'format renders an unknown placeholder as empty (no {var} leak to the user)',
        () {
      final s = {
        'skillId': 'x',
        'steps': {
