/// The manual Library facade (Spec 07 §5.5 tap-to-edit + the Learned phrases showcase, G-49).
...
/// Wraps a real repository and can fail record writes — a full disk / revoked folder permission.
...
String _taskId(Session s, String desc) => s.store.values.firstWhere(
    (r) => r['typeId'] == 'task' && r['description'] == desc)['id'] as String;

void main() {
  group('a torn learned-corpus file must not brick startup', () {
    test(
        'half-written corpus-learned.json degrades to "nothing learned", app still opens',
        () async {
      ...
      File('$dir/corpus-learned.json')
          .writeAsStringSync('[{"skillId":"create-task","templa');
    ...
    test('a torn SEED corpus degrades routing but still opens, and says so',
        () async {
      final dir = makeTempDataDir();
      File('$dir/corpus.json')
          .writeAsStringSync('[{"skillId":"create-task","temp');
      final s = Session(dir, clock: _now, cloud: _NoCloud());
      await s.init(
          retrieval:
              false); // must not throw — a launch crash is unrecoverable on a phone
      expect(s.router.corruptFiles, isNotEmpty,
          reason: 'surfaced for repair, not silent');
      expect(s.router.corpus, isEmpty);
    });

    test(
        'saveConfig writes atomically (a torn config would lose dataDir + the API key)',
        () async {
      ...
      expect(
        loadConfig(configPath: '$dir/config.json', environment: const {})
            .apiKey,
        'sk-test',
      );
    });

    test('learned-corpus writes are atomic (no .tmp left, file always parses)',
        () async {
      final dir = makeTempDataDir();
      final s = Session(dir, clock: _now, cloud: _NoCloud());
      await s.init(retrieval: false);
      s.repo.appendCorpusLearned({
        'skillId': 'create-task',
        'template': 'chuck {description:text} on the list'
      });
      expect(File('$dir/corpus-learned.json.tmp').existsSync(), isFalse);
      expect(jsonDecode(File('$dir/corpus-learned.json').readAsStringSync()),
          isA<List>());
    });
  });
  ...
  group('editable data view — write failures stay values, never exceptions',
      () {
    test(
        'a null value CLEARS an optional field instead of writing the string "null"',
        () async {
      final s = await _s();
      ...
      expect(r.ok, isTrue);
      expect(s.store[id]!['dueAt'], isNot('null'),
          reason: 'interpolating null must not become junk data');
      expect(s.store[id]!.containsKey('dueAt') && s.store[id]!['dueAt'] != null,
          isFalse);
    });

    test(
        'a failing repository returns a truthful partial value rather than throwing across the UI seam',
        () async {
      final dir = makeTempDataDir();
      final failing = _FailingRepo(FileStorageRepository(dir));
      final s = Session(dir, clock: _now, cloud: _NoCloud(), storage: failing);
      await s.init(retrieval: false);
      await s.handle('add buy milk to my list');
      final id = _taskId(s, 'buy milk');
      ...
      expect(r.ok, isTrue,
          reason:
              'the in-memory edit happened and has a durable recovery intent');
      expect(r.message, contains("couldn't finish saving"));
      final d = await s.deleteRecord(id);
      expect(d.ok, isTrue,
          reason:
              'the in-memory delete happened and has a durable recovery intent');
      expect(d.message, contains("couldn't finish saving"));
    });
  });

  group('editable data view — facade', () {
    test('editField updates a value, validated, and voice undo reverses it',
        () async {
      final s = await _s();
      await s.handle('add buy milk to my list');
      final id = _taskId(s, 'buy milk');
      ...
      final undo =
          await s.handle('undo that'); // the SAME journal as a spoken write
    ...
    test('editField coerces a decimal field and rejects non-numbers', () async {
      ...
      final bad =
          await s.editField(w['id'] as String, 'distance', 'not a number');
    ...
    test('editField on a non-schema field is refused (no silent write)',
        () async {
      final s = await _s();
    ...
    test('editField coerces a boolean field (B4)', () async {
      ...
      expect(r.ok, isTrue);
      expect(s.store[id]!['completed'],
          isTrue); // a real bool, not the string "true"
    });

    test('editField stores a date value (picker ISO passthrough, B4)',
        () async {
      final s = await _s();
    ...
    test(
        'a manual edit is NOT reversed by a following voice correction (review #2)',
        () async {
      final s = await _s();
      await s.handle('log a 3k run'); // a voice write (workout)
      await s.handle('add buy milk to my list');
      final id = _taskId(s, 'buy milk');
      await s.editField(id, 'description',
          'buy oat milk'); // manual edit — supersedes "the last turn"
      await s.handle('no, I meant buy bread'); // a voice correction
      expect(s.store[id]!['description'],
          'buy oat milk'); // the manual edit survived — not reversed
    });

    test(
        'targeted undo (undoById) reverses the delete even after a later write (review #4)',
        () async {
      final s = await _s();
      ...
      await s
          .handle('add another to my list'); // a later write lands on the ring
      final msg = await s.undoById(del.undoId!);
      expect(msg.toLowerCase(), contains('undone'));
      expect(s.store.containsKey(id),
          isTrue); // the DELETE was reversed, not the later add
      expect(s.store.values.any((r) => r['description'] == 'another'),
          isTrue); // later add survived
    });
  ...
  group('learned phrases — showcase + forget/restore', () {
    test('a learned template shows up as a LearnedFlow with a human target',
        () async {
      final s = await _s();
      ...
      expect(f.targetLabel,
          'List your tasks'); // skill displayName, not the raw skillId
      expect(f.isGenerative, isFalse);
    });

    test('forgetLearnedFlow drops it; restoreLearnedFlow brings it back',
        () async {
      final s = await _s();
      s.router.addLearned('list-tasks', 'show my todo list');
      s.repo.appendCorpusLearned({
      ...
      expect(s.learnedFlows.any((f) => f.template == 'show my todo list'),
          isFalse);
      s.restoreLearnedFlow(token!);
      expect(
          s.learnedFlows.any((f) => f.template == 'show my todo list'), isTrue);
    });
    ...
    test('forget -> restore of a GENERATIVE-kind learned flow round-trips (B6)',
        () async {
      final s = await _s();
      const tmpl = 'ideas for a present for {contact:entity}';
      s.router.restore({'generativeKind': 'gift_ideas', 'template': tmpl});
      s.repo.appendCorpusLearned(
          {'generativeKind': 'gift_ideas', 'template': tmpl});
      final flow = s.learnedFlows.firstWhere((f) => f.template == tmpl);
      expect(flow.isGenerative, isTrue);
      expect(flow.targetLabel,
          'Gift ideas'); // humanized generativeKind, not a skillId
      final token = s.forgetLearnedFlow(tmpl);
      expect(token, isNotNull);
      expect(token!['generativeKind'],
          'gift_ideas'); // token preserves the generative shape
      expect(s.learnedFlows.any((f) => f.template == tmpl), isFalse);
      s.restoreLearnedFlow(token);
      expect(s.learnedFlows.any((f) => f.template == tmpl && f.isGenerative),
          isTrue);
    });

    test('double restore does not duplicate a learned flow (review #13)',
        () async {
      final s = await _s();
      s.router.addLearned('list-tasks', 'show my todos');
      s.repo.appendCorpusLearned(
          {'skillId': 'list-tasks', 'template': 'show my todos'});
      final token = s.forgetLearnedFlow('show my todos')!;
      s.restoreLearnedFlow(token);
      s.restoreLearnedFlow(
          token); // second restore must be a no-op, not a duplicate
      expect(
          s.learnedFlows.where((f) => f.template == 'show my todos').length, 1);
    });
  });
}
