...
void main() {
  group('log-interaction', () {
    test('creates the contact if absent and logs an interaction dated today',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      final r = await s.handle('called Sarah');
      expect(r, contains('Sarah'));
      final contacts =
          s.store.values.where((x) => x['typeId'] == 'contact').toList();
      expect(contacts.length, 1);
      expect(contacts.single['displayName'], 'Sarah');
      final ints =
          s.store.values.where((x) => x['typeId'] == 'interaction').toList();
      expect(ints.length, 1);
      expect(ints.single['at'], '2026-07-06');
      expect(ints.single['subject'], contacts.single['id']); // entity ref wired
    });

    test('captures a note and reuses an existing contact (no duplicate)',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      await s.handle('called Sarah');
      await s.handle('called Sarah about the school trip');
      expect(s.store.values.where((x) => x['typeId'] == 'contact').length,
          1); // reused
      final ints =
          s.store.values.where((x) => x['typeId'] == 'interaction').toList();
    ...
    test(
        'captures a past date: "called Sarah yesterday" (gap: dated interaction)',
        () async {
      final s =
          await _session(makeTempDataDir(), clock: _d('2026-07-06')); // Monday
      await s.handle('called Sarah yesterday');
      final ints =
          s.store.values.where((x) => x['typeId'] == 'interaction').toList();
      expect(ints.single['at'], '2026-07-05'); // yesterday, not today
    });

    test('"called Sarah on last friday" back-dates to the prior friday',
        () async {
      final s = await _session(makeTempDataDir(),
          clock: _d('2026-07-06')); // Monday 07-06
      await s.handle('called Sarah on last friday');
      final ints =
          s.store.values.where((x) => x['typeId'] == 'interaction').toList();
      expect(ints.single['at'], '2026-07-03');
    });

    test('a BARE weekday back-dates too (review #6): "called Sarah on friday"',
        () async {
      final s = await _session(makeTempDataDir(),
          clock: _d('2026-07-06')); // Monday 07-06
      await s.handle(
          'called Sarah on friday'); // a PAST interaction -> the PREVIOUS friday
      final ints =
          s.store.values.where((x) => x['typeId'] == 'interaction').toList();
    ...
    test(
        'reverse fact search: "who do I know that likes hiking" (gap: reverse search)',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
  ...
  group('upcoming plans vs past interactions (dogfood: future logged as past)',
      () {
    test('a plan is future-tensed, queryable, and NOT counted as "last talked"',
        () async {
      final dir = makeTempDataDir();
      final s = await _session(dir, clock: _d('2026-07-06')); // Monday
      await s.handle('called Sarah'); // a PAST interaction, today
      final r =
          await s.handle('seeing Sarah on friday'); // a FUTURE plan, 07-10
      expect(r.toLowerCase(), contains("you're seeing sarah"));
      final plan = s.store.values.firstWhere(
          (x) => x['typeId'] == 'interaction' && x['planned'] == true);
      expect(plan['at'], '2026-07-10');
      // "when am I seeing X next" surfaces the plan
      expect(await s.handle('when am i seeing Sarah next'),
          contains('2026-07-10'));
    ...
    test('no plans -> a clear message', () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      await s.handle('called Sarah');
      expect((await s.handle('when am i seeing Sarah next')).toLowerCase(),
          contains('upcoming plans'));
    });
  });

  group('neglected-contacts (gap: who haven\'t I talked to)', () {
    test('surfaces stale + never-contacted people, hides the recent ones',
        () async {
      final dir = makeTempDataDir();
      // Ed: called long ago (2026-05-01, > 30 days before the 07-06 query)
      final sOld = await _session(dir, clock: _d('2026-05-01'));
      await sOld.handle('called Ed');
      ...
      // Sarah: called today (recent)
      await sMid.handle('called Sarah');
    ...
    test('clear message when everyone is recent', () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      await s.handle('called Sarah');
      expect((await s.handle('who have i been neglecting')).toLowerCase(),
          contains('good touch'));
    });
  });

  group('last-interaction (max date via reduction, not insertion order)', () {
    test('reports the most recent date even when logged out of order',
        () async {
      final dir = makeTempDataDir();
      // log the LATER interaction first, then an earlier one, over the same folder
      await (await _session(dir, clock: _d('2026-07-08')))
          .handle('called Sarah about the trip');
      await (await _session(dir, clock: _d('2026-07-06')))
          .handle('called Sarah');

      final ask = await _session(dir, clock: _d('2026-07-10'));
      final r = await ask.handle('when did i last talk to Sarah');
      expect(
          r, contains('2026-07-08')); // the max, not the last inserted (07-06)
      expect(r, contains('Wednesday')); // 2026-07-08 weekday
    });

    test(
        'unknown contact -> says so; known contact with no interactions -> says so',
        () async {
      final dir = makeTempDataDir();
      final s = await _session(dir, clock: _d('2026-07-06'));
      expect(await s.handle('when did i last talk to Nobody'),
          contains("don't have"));
  ...
  group('per-person dates / anniversaries (gap #8)', () {
    test('set + query an anniversary, without colliding with birthdays',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      ...
      expect(
          await s.handle("when is Sarah's birthday"), contains('2026-07-16'));
    });

    test('unknown date label says so', () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      await s.handle('called Sarah');
      expect((await s.handle("when is Sarah's anniversary")).toLowerCase(),
          contains("don't know"));
    });
  });

  group('birthdays', () {
    test('set-birthday creates a contact with the birthday (month-name date)',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
    ...
    test('set-birthday updates an existing contact without duplicating',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      await s.handle('called Sarah'); // creates the contact
    ...
    test('contact-age computes the current age from the birthday (gap: age)',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      await s.handle(
          "Sarah's birthday is march 3"); // resolves in 1990? no — current year
    ...
    test('contact-age: birthday not yet reached this year counts one less',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      await s.handle('called Ben'); // create contact
      final c = s.store.values.firstWhere((x) => x['typeId'] == 'contact');
    ...
    test('when-birthday: unknown contact / no birthday set', () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      expect(
          await s.handle("when is Nobody's birthday"), contains("don't have"));
      await s.handle('called Amir');
      expect(await s.handle("when is Amir's birthday"), contains("don't know"));
    });

    test('upcoming-birthdays lists within 30 days and excludes far-off ones',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
    ...
    test('upcoming-birthdays: none coming up', () async {
      ...
      expect(
          await s.handle('any birthdays coming up'), contains('No birthdays'));
    });
  });

  group('list-interactions', () {
    test('lists logged interactions with dates + notes, newest data intact',
        () async {
      final dir = makeTempDataDir();
      await (await _session(dir, clock: _d('2026-07-06')))
          .handle('called Sarah about the school trip');
      await (await _session(dir, clock: _d('2026-07-08')))
          .handle('called Sarah');
    ...
    test('unknown contact / a contact with no interactions', () async {
      ...
      await s
          .handle('note that Priya loves climbing'); // contact, no interaction
      expect(await s.handle('our history with Priya'),
          contains("haven't logged any"));
    });
  });

  group('on-open birthday nudges (derived, no new skill)', () {
    test('a birthday within a week nudges on open; a far-off one does not',
        () async {
      final dir = makeTempDataDir();
      final s = await _session(dir, clock: _d('2026-07-06'));
      ...
      final reopened =
          await _session(dir, clock: _d('2026-07-06')); // fresh open re-derives
      final nudges = reopened.pendingNudges();
      expect(nudges.any((n) => n.contains('Sarah') && n.contains('in 4 days')),
          isTrue);
      expect(nudges.any((n) => n.contains('Mia')), isFalse);
      expect(
          nudges.every((n) => n.startsWith('🎂') || n.startsWith('⏰')), isTrue);
    });

    test(
        'pure helper: today/tomorrow phrasing, skips missing birthdays, sorted soonest-first',
        () {
  ...
  group('remember-relationship (offline coverage of "X is Y\'s Z")', () {
    test(
        '"remember that Mia is Sarah Mitchell\'s daughter" creates the relationship, queryable both ways',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      final r =
          await s.handle("remember that Mia is Sarah Mitchell's daughter");
      expect(r, contains("Mia is Sarah Mitchell's daughter"));
      final contacts = s.store.values
          .where((x) => x['typeId'] == 'contact')
          .map((c) => c['displayName'])
          .toList();
      ...
      expect(
          ...
          1);
      expect(
          await s.handle('who is Sarah related to'), contains('daughter: Mia'));
      expect(await s.handle('who is Mia related to'),
          contains('daughter of Sarah Mitchell'));
    });

    test(
        'anchored predicate without "remember": "Sarah is allergic to peanuts" (gap #5)',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      await s.handle('called Sarah'); // Sarah is now a known contact
      final r = await s.handle('Sarah is allergic to peanuts');
      expect(r, contains('is allergic to peanuts'));
      expect(await s.handle('what do i know about Sarah'),
          contains('is allergic to peanuts'));
    });

    test('self-relative capture: "my sister Emma loves pottery" (gap #3)',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
    ...
    test(
        'control: a trait about a NON-contact ("France likes wine") is ignored',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      await s
          .handle('France likes wine'); // France isn't a contact -> no capture
      expect(s.store.values.where((x) => x['typeId'] == 'contact').isEmpty,
          isTrue);
    });

    test(
        'a plain "remember that X <fact>" still records a fact, not a relationship',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      await s.handle('remember that Mia loves chess');
      expect(
          await s.handle('what do i know about Mia'), contains('loves chess'));
      expect(s.store.values.where((x) => x['typeId'] == 'contact_relationship'),
          isEmpty);
    });

    test(
        'recall surfaces relationships too (dogfood: spouse was stored but invisible)',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      await s.handle("remember that Mia is Sarah's daughter");
      // "tell me about X" now includes relationships, not just facts
      expect(await s.handle('what do i know about Sarah'),
          contains('daughter: Mia'));
      expect(await s.handle('what do i know about Mia'),
          contains('daughter of Sarah'));
    ...
    test(
        'relation-between answers "how are X and Y related" in both directions (gap #2)',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      await s.handle("remember that Mia is Sarah's daughter");
      expect(await s.handle('how are Sarah and Mia related'),
          contains("Mia is Sarah's daughter"));
      expect(await s.handle('how are Mia and Sarah related'),
          contains("Mia is Sarah's daughter"));
    });
  ...
  group('list-relations', () {
    test(
        'shows a person\'s relationships with the linked contact name resolved',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
    ...
    test('resolves the reverse direction too ("who is Mia related to")',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      await s.handle("remember that Mia loves art and she is Sarah's daughter");
      final r = await s.handle('who is Mia related to');
      expect(
          r, contains('daughter of Sarah')); // reverse: Mia is Sarah's daughter
    });

    test('unknown contact / a contact with no relationships', () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      expect(
          await s.handle('who is Nobody related to'), contains("don't have"));
      await s.handle('called Alex'); // a contact, but no relationships noted
      expect(await s.handle("who are Alex's relatives"),
  ...
  group('partial name matching + disambiguation (people reads)', () {
    test('a first name resolves to the full-name contact', () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      await s.handle('called Sam Rivera about chess');
      final r = await s
          .handle('when did i last talk to Sam'); // "Sam" -> "Sam Rivera"
      expect(r, contains('2026-07-06'));
      expect(r.contains("don't have"), isFalse);
    });

    test(
        'an ambiguous first name asks which one — candidates listed, no raw error',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      await s.handle('called Sam Rivera');
      await s.handle('called Sam Chen');
    ...
    test('an exact name still wins over a partial one (no false ambiguity)',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      await s.handle('called Sam'); // contact "Sam"
      await s.handle('called Sam Rivera'); // contact "Sam Rivera"
      final r = await s.handle('when did i last talk to Sam');
      expect(
          r.contains('more than one'), isFalse); // exact "Sam" resolves cleanly
    });

    test(
        'find-or-create REUSES an overlapping-name contact — no duplicate (user-reported)',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      await s.handle('called Sam Rivera');
      await s.handle(
          'called Sam'); // "Sam" is a whole-word subset -> reuse Sam Rivera
      final names = s.store.values
          .where((x) => x['typeId'] == 'contact')
          .map((c) => c['displayName'])
          .toList();
    ...
    test(
        'a relationship never collapses two overlapping names into a self-relationship (review #1)',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      await s.handle('called Maria Elena'); // existing contact "Maria Elena"
      await s.handle(
          "remember that Maria is Maria Elena's mother"); // "Maria" must NOT resolve to "Maria Elena"
      final names = s.store.values
          .where((x) => x['typeId'] == 'contact')
          .map((c) => c['displayName'])
          .toSet();
      expect(names, {'Maria Elena', 'Maria'}); // distinct people, both present
      final rel = s.store.values
          .firstWhere((x) => x['typeId'] == 'contact_relationship');
      expect(rel['from'] == rel['to'], isFalse); // not a self-relationship
    });

    test(
        'an ambiguous name on a WRITE clarifies instead of guessing (review #2)',
        () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      await s.handle('called John Smith');
      await s.handle('called John Doe'); // two DISTINCT Johns
      final r = await s
          .handle('remember that John likes chess'); // "John" is ambiguous
      expect(r.toLowerCase(), contains('more than one'));
      expect(s.store.values.where((x) => x['typeId'] == 'contact_fact'),
          isEmpty); // nothing written on the guess
    });

    test('a non-token name stays distinct — "Sam" is NOT "Samantha"', () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      await s.handle('called Samantha');
      await s.handle(
          'called Sam'); // "Sam" is not a whole word in "Samantha" -> distinct
      final names = s.store.values
          .where((x) => x['typeId'] == 'contact')
          .map((c) => c['displayName'])
          .toSet();
  ...
  group('people-loop routing', () {
    test('"called X about Y" -> log-interaction with a note', () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      await s.handle('called Dad about the car');
      final ints =
          s.store.values.where((x) => x['typeId'] == 'interaction').toList();
      expect(ints.single['note'], 'the car');
    });
    test('"FaceTimed X" and "called X" both log typed interactions', () async {
      final s = await _session(makeTempDataDir(), clock: _d('2026-07-06'));
      await s.handle('facetimed Mia');
      await s.handle('called Grandma');
      expect(
          s.store.values.where((x) => x['typeId'] == 'interaction').length, 2);
